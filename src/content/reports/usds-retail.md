...
*Revision history: 2026-08-25 — **the cancellation leg of the 48-hour delay is qualified; the delay itself is re-confirmed; no score change.** Re-measured on the DSPause at `0xbe286431…38f3`: `delay()` **172,800 seconds exactly**, `owner()` the zero address, and `getMinDelay`, `MINIMUM_DELAY` and `MIN_DELAY` all revert. **The delay is real and this report was right about it.** ⚠️ **What was not established until now: `plot()` and `drop()` — schedule and cancel — answer to the same DSAuth authority.** There is no distinct canceller held by an independent party, so **whoever can queue an action can also withdraw one, and nobody else can do either.** Against the case a timelock exists for — a compromised or captured proposer — **the cancel power belongs to the attacker.** This report previously said *"a malicious governance action can be cancelled during the 48-hour window,"* which is literally true and reads as a protection it is not. **The 48 hours buys notice, not interruption:** two days of public warning a holder can act on, with nobody independent able to stop the action inside it. **Forty-eight hours of notice nobody independent can act on is notice, not protection.** **Scores unchanged (Issuer 7.0, Overall 7.5), and the cohort decision survives the correction** — the discriminating variable across this coverage was always whether a *holder* gets a reaction window, not whether a third party can intervene, and USDS still gives one where the docked cases give none. The clause has also been qualified where it was cited on [USDT](/reports/usdt/) and [AUSD](/reports/ausd/) as the reason USDS was not docked. `last_verified` is bumped to 2026-08-25 for the governance material; **the collateral mix, allocator shares and supply figures on this page still date from the 2026-07-09 pass.** 2026-08-23 — admin path re-verified on-chain; **no score changes, and the report's central governance claim is confirmed rather than weakened.** The 48-hour Governance Security Module delay reads exactly 172,800 seconds, the pause contract's `owner()` is the zero address, its authority is the governance Chief, and no bypass role exists on its function surface — so "a full 48 hours", "no owner backdoor" and "no single key can bypass it" all hold, and the Issuer axis is unchanged on this report and on [sUSDS](/reports/susds/). Two narrow corrections, both sharpening rather than hedging. **"The only authorized controller" was wrong**: there is a second write-authorized contract, the DAI/USDS conversion contract at `0x3c0f8950…7feb`, whose entire surface is `join` and `exit` — it can mint and burn on conversion and holds no governance or upgrade power. Mechanically constrained, but "only" was still the wrong word, and a reader who found the second one had grounds to doubt the parts that were true. **And 48 hours is a setting rather than a floor**: `setDelay` exists with no minimum or maximum in the bytecode, so governance can lower it — which does not weaken the warning, because any such change must itself pass through the delay in force. Stated now as "48 hours today, changeable only through the same 48-hour path", with a watch item added for a spell that lowers it. The Audits section was also reframed: it filed the upgradeable contract under "design choices, not contract bugs", which is true but reads as dismissing admin risk — an upgrade path is a contract risk even when it is not a contract bug, and here it has been measured and passes. `last_verified` is deliberately **not** bumped: only the admin path was re-read on 2026-08-23, and the collateral and supply figures in this body still date from the 2026-07-09 pass. 2026-07-09 — refreshed collateral model (Star-allocator system now ~52%, crypto CDPs ~7%; added credit-sleeve + non-USDC-stable detail). Scores unchanged.*
</content>
</invoke>
