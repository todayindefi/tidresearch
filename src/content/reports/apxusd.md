...
**apxUSD broke its peg in June 2026 and ran below par on collateralization until 2026-09-11.** That event reshapes most of this report — see *The June 2026 depeg* directly below. Live peg, reserves, and collateralization are on the [dashboard](https://tidresearch.com/dashboards/?asset=apxusd).
...
>
> ⚠️ **One composition fact worth carrying, because it bears on what can be checked at all: of the $183.0M STRC bucket, only $67.6M — 36.9% — is on-chain STRCx. The remaining $115.5M, 63.1%, is issuer-stated.** **The largest single reserve line is majority off-chain**, which is why the independently verifiable portion of this reserve is the smaller one.
>
...
That correction — more STRC concentration than previously scored, and a thinner true-cash cushion — combined with a collateral ratio that ran **below par continuously from June until 2026-09-11** is why the **Backing axis sits at 2.0**. ⚠️ **That continuity has ended and the score has not been re-derived on it** — the axis also rests on the concentration and cash-cushion findings above, which are unchanged, and a re-rate needs its own pass rather than a reaction to three days of readings. ⚠️ **apxUSD crossed back above par on 2026-09-11 and has held there since** — above 100% on the netted collateral ratio, on the gross basis, and on an independent conservative re-mark. ⚠️ **Read the level carefully: it runs roughly 100.0–101.0%, with a median around 100.25% — about 25 basis points of surplus.** **That is a recovery, not a buffer**, it is three days old, and it sits on a book that was in deficit the week before. ✅ **What it removes is the realised undercollateralisation; it does not remove the backing concern**, which rests on concentration and on what the reserve is made of rather than on the ratio alone. A 50% STRC writedown would still leave the cash and POL portion, but that cushion is thinner than the gross numbers implied. Strategy's buyback program has since lifted the STRC *mark* substantially, which carried the attested ratio back up to roughly 97% and then **above par on 2026-09-11** — real repair, and the reason the trajectory reads as recovery. ⚠️ **It has not lifted this axis, and the reason is no longer the level.** **The ratio is above par but by about 25 basis points and for three days, and the improvement came from an issuer bid on the collateral rather than from Apyx restocking its own reserve.** The recovery here is borrowed from Strategy's balance sheet, and Strategy's buyback authorization is finite.
...
- STRC + SATA are held at Alpaca brokerage in the name of Preference Foundation (or a subsidiary). STRCx (the on-chain tokenized form of STRC) sits in [`0x37b0779a…323a555`](https://etherscan.io/address/0x37b0779a66edc491df83e59a56d485835323a555), a 3-of-6 Gnosis Safe with the same six owners as Apyx's other admin Safes (identified on-chain; the 4/30 balance matches the Wolf attestation within 1%). ⚠️ **That sameness was re-measured on 2026-08-25 and is exact, across three Safes rather than two:** this custody Safe, the 4-of-6 token-governance Safe [`0xABdd8c8e…65e96`](https://etherscan.io/address/0xABdd8c8eE69e5F5180eb9352AEFFC5CeeAD65e96), and the bridge governor [`0xf9862EfC…3cE2`](https://etherscan.io/address/0xf9862EfC1704aC05e687f66E5cD8c130E5663cE2). ⚠️ **Re-read on-chain 2026-09-09, that sameness is no longer exact and the bridge is the reason:** the token-governance and custody Safes still hold the **identical six owners**, but the bridge Safe moved to **4-of-7** at block 25,877,589 on 2026-08-31 — one owner added, threshold raised — so it now holds those six **plus one exclusive signer**.

**Six people still hold three hats.** Four of them satisfy token governance, **three of them satisfy this custody Safe**, and four of them satisfy the bridge, so **the six reach all three doors and the seventh signer opens none they cannot.** ⚠️ **The 3-of-6 custody hat is now the only one with both a low quorum and no delay: any three of those six can move this collateral without touching the 72-hour delay that governs the token.** The bridge closed its quorum gap on 08-31 and keeps its delay gap. The owner-set identity and thresholds are measured on-chain; the role attributions are read from this coverage's records. See the [apyUSD report](/reports/apyusd/) for the full write-up and the correction history behind it. The Safe has no time-delay on transactions — same admin gap as the cross-chain bridge layer.
...
- Needing instant guaranteed exit at $1 — the documented redemption mechanism is opaque, secondary depth is issuer-discretionary, and the collateral ratio, below par from June, crossed back above it on 2026-09-11 at a thin margin.
...
- **2026-06-29 — STRC soft-floor reframe** following Strategy's 8-K.
- **2026-06-25 — depeg deepened.**
