...
⚠️ **And the two signer sets appear to overlap, which is the part neither this report nor its sibling has carried — but read the provenance before you weight it.** Walked on 2026-08-23, the 4-of-6 token-governance Safe and the 3-of-6 bridge Safe **shared only 3 of 6 owners** — an overlap that would matter, because the bridge Safe's threshold is three: **the three shared signers would satisfy the bridge multisig on their own.**

⚠️ **That comparison is a single observation and this coverage cannot currently re-derive it.** The bridge Safe is `0x37b0779a66edc491df83e59a56d485835323a555`, and its **3-of-6 configuration verifies live**. The token-governance Safe's address is **not recorded in full anywhere in this coverage** — only truncated — so the owner-set intersection cannot be re-run from our own records. **A truncated address is not a record of an address; it is a record that someone once saw one.**

**So treat the overlap as reported rather than established**, and note what that means for the score: the axis is held **because the finding is unverified, not because it has been judged harmless.** If it reproduces, the separation between the two admin layers is thinner than two differently-sized multisigs suggest, and that is a real drag this report has not priced.

**So the separation between the two layers is thinner than two differently-sized multisigs suggest.** The token path still requires four signatures behind a 72-hour timelock, and that is unchanged and real. The bridge path requires three — and those three can be drawn entirely from people who also sit on the token Safe, with no delay on that route.

⚠️ **This is the same shape as the [frxUSD](/reports/frxusd/) finding, and it generalises: per-chain and per-contract rows can each be correct while the composite is the risk, and the composite exists only in the comparison.** Reading the two Safes separately — 4-of-6 here, 3-of-6 there — tells you nothing about how many distinct people stand between an attacker and the bridge. **That number only appears when you intersect the sets, and nothing in either row prompts you to.**
...
- **2026-08-25 — signer-set overlap published; a clean-pass result recorded; scores held at 3.0.** ⚠️ **The 4-of-6 token-governance Safe and the 3-of-6 bridge Safe were walked as sharing only 3 of 6 owners — enough to satisfy the bridge multisig outright**, since its threshold is three. ⚠️ **Qualified 2026-08-25: that is a single observation this coverage cannot re-derive.** The bridge Safe (`0x37b0779a…a555`) verifies live at 3-of-6, but the **token-governance Safe's address is recorded only in truncated form**, so the owner-set intersection cannot be re-run from our records. **A truncated address is not a record of an address; it is a record that someone once saw one.** The finding stands as reported rather than established, and the axis is held **because it is unverified, not because it has been judged harmless.** So the separation between the two layers is thinner than two differently-sized multisigs suggest: the token path still needs four signatures behind a 72-hour timelock, while the bridge path needs three, and those three can be drawn entirely from the token Safe with no delay on that route. **Same shape as the [frxUSD](/reports/frxusd/) finding** — per-contract rows can each be correct while the composite is the risk, and **the composite exists only in the comparison**: neither row prompts a reader to intersect the sets, and the number that matters appears nowhere until they do. **Also recorded, from a 2026-08-17 internal review that never reached this page: a hold at 3.0 in which the discount-drift alert fired correctly and changed nothing.** That is a checked-and-held result and is published as one, because a reader cannot otherwise distinguish *still true* from *nobody looked*. `last_verified` is bumped to 2026-08-25 for the admin material; the NAV, discount and exit figures still date from the earlier passes recorded below.
