...
⚠️ **And the two Safes are not two groups of people. They are the same six people at two thresholds.** Measured on Ethereum: the 4-of-6 token-governance Safe [`0xABdd8c8e…65e96`](https://etherscan.io/address/0xABdd8c8eE69e5F5180eb9352AEFFC5CeeAD65e96) and the 3-of-6 bridge Safe [`0x37b0779a…a555`](https://etherscan.io/address/0x37b0779a66edc491df83e59a56d485835323a555) share **six of six owners** — **zero signers exclusive to either side.**

**Any three of those six clear the bridge; any four of the same six clear token governance.** That is not two authorities with overlapping membership. **It is one membership with two quorums**, and the lower quorum is the one with no delay attached.

⚠️ **Which means the 72-hour timelock protects a path those same people can decline to use.** The token route still requires four signatures and still waits three days, and that is unchanged and real. But the bridge route requires three signatures from the same roster, moves apyUSD across chains, and waits for nothing. **A delay is only a delay if the people it binds have no faster door.**

⚠️ **Correcting this report: an earlier version of this section said the two Safes "share only 3 of 6 owners." That was wrong, and wrong in the direction that flattered the subject** — it described a partial separation that does not exist. It was published on a single walk that neither this coverage nor its source could re-derive, and it was flagged here as reported-rather-than-established for exactly that reason. **That flag is why it was re-checked instead of aging into fact.** When checked it was not merely unverified but false, and it *understated* the concentration.

⚠️ **This is the same shape as the [frxUSD](/reports/frxusd/) finding, and it generalises: per-contract rows can each be correct while the composite is the risk, and the composite exists only in the comparison.** Reading the two Safes separately — 4-of-6 here, 3-of-6 there — tells you nothing about how many distinct people stand between an attacker and the bridge. **That number only appears when you intersect the sets, and nothing in either row prompts you to.** Here the intersection returns six, and the honest count of distinct people standing between an attacker and both layers is not ten. **It is six.**
...
- **2026-08-25 — signer-set overlap published, then corrected the same day; a clean-pass result recorded; scores held at 3.0.** ⚠️ **Measured: the 4-of-6 token-governance Safe [`0xABdd8c8e…65e96`](https://etherscan.io/address/0xABdd8c8eE69e5F5180eb9352AEFFC5CeeAD65e96) and the 3-of-6 bridge Safe [`0x37b0779a…a555`](https://etherscan.io/address/0x37b0779a66edc491df83e59a56d485835323a555) share six of six owners — zero signers exclusive to either side.** **One membership at two quorums, not two authorities**: any three of the six clear the bridge with no delay, any four of the same six clear token governance behind the 72-hour timelock. **The timelock protects a path those same people can decline to use.** ⚠️ **This entry first published the overlap as "only 3 of 6 owners" — wrong, and wrong in the direction that flattered the subject**, since it implied a partial separation that does not exist. It went up on a single walk that neither this coverage nor its source could re-derive, and was flagged here as reported-rather-than-established for exactly that reason. **That flag is the whole reason it was re-checked rather than aging into fact** — and on re-check it was not merely unverified but false, and it understated the concentration. The address was in the source repository in full the entire time; both sides searched the representation they expected it to live in and neither searched the other. **Same shape as the [frxUSD](/reports/frxusd/) finding** — per-contract rows can each be correct while the composite is the risk, and **the composite exists only in the comparison**: neither row prompts a reader to intersect the sets, and the number that matters appears nowhere until they do. **Also recorded, from a 2026-08-17 internal review that never reached this page: a hold at 3.0 in which the discount-drift alert fired correctly and changed nothing.** That is a checked-and-held result and is published as one, because a reader cannot otherwise distinguish *still true* from *nobody looked*. `last_verified` is bumped to 2026-08-25 for the admin material; the NAV, discount and exit figures still date from the earlier passes recorded below.
