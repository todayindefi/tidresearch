...
> **Current state (2026-08-23) — the ~84% above is a June figure and the collateral has recovered a long way since. No score change.** apxUSD's collateral ratio reads **98.02% on the issuer-attested basis** against a supply of about **$312.1M** — a shortfall of roughly **$6.2M** — and **98.59% on our stricter independent lower bound**, a shortfall of about **$4.4M**. **Quote whichever you like, but pair each ratio with its own deficit:** the two bases mark the same book differently, so an attested ratio alongside a lower-bound dollar figure understates the gap by about $1.8M. **That is still below par, so the sentence above this block stands** — the terminal asset remains under-collateralized and the recovery gate (par or better, sustained) is not met, which is why nothing re-rates. **The direction is the news, not the level.** ⚠️ **And note which way the two bases sit:** the independent lower bound is *higher* than the issuer-attested figure, because it re-marks only the on-chain STRCx slice at underlying STRC value while the headline marks the larger raw-STRC slice at a price that is not disclosed. **The portion we can check independently marks better than the attestation; the portion we cannot check is the larger one.** That is a verifiability point, not a solvency one. Two live conditions this report's older sections do not carry: STRC sits at about **60% of Apyx's reserves**, above the 55% single-issuer threshold we flag elsewhere; and depth on the Jupiter-routed STRCx venue is thin at roughly **$180K**, which bears directly on the exit arithmetic below. Live figures on the [dashboard](https://tidresearch.com/dashboards/?asset=apyusd).
