...
**The evidence that the move was authorised is behavioural, not cryptographic.** A single ULTRA walked the entire route — treasury → proxy → EOA → back — between 17 and 20 August. The full 35.2M followed the identical path **1 hour 51 minutes** after that rehearsal completed, and the test token was returned to the treasury, which is why the treasury balance reads exactly 1.000000. **An operator dry-runs a migration with one token and hands it back; an attacker does not.** That is a strong inference and it is not proof.

**Control cannot be read from the chain.** The destination is a 708-byte ERC-1967 proxy whose **admin slot is empty — a UUPS pattern**, so upgrade authority lives inside the implementation and is not enumerable from storage. `owner()`, `name()`, `symbol()` and `asset()` all return nothing.

**So: located, custody structurally identified, control unattributed.** The question has narrowed from *"where did $37M go"* to **"who can upgrade `0x989cc593…`"** — which is specific, checkable, and not yet checked.

⚠️ **Why no score moves on this, and what would change it.** The sign of this finding still turns on that attribution, and the two branches point opposite ways. If the destination is Theo's, this is a **reserve-tracker coverage gap** — the tracker reads the old address, the assets are understated by about $37M, and the correction belongs in our monitoring rather than in this report. If it is not Theo's, it is a materially more serious report than this one. The behavioural evidence favours the first and this report says so. **The single input that would settle it is identifying who holds upgrade authority over `0x989cc593…`.** Until then no restated backing figure is published here, because publishing a number against evidence that could reverse direction is a worse error than publishing none.
