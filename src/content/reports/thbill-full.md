...
**Contract Risk Score: 5.5/10** — Single audit with clean findings (no Crit/High), competent architecture, a nested multisig structure (4-of-6 on Ethereum down to 3-of-4 on Base/HyperEVM, same owner set truncated) with transparent roles, and proper 3-DVN custom config across every peered cross-chain pathway. Deductions for: single audit firm, no bug bounty, OFT adapter likely post-audit, no timelock, upgradeable core, short Lindy. Trust holes: the MPC scheme (threshold, co-signers, ceremony) is undocumented and should be treated as opaque multisig of unknown composition; the distinct EIP-1967 proxy-admin role is not verifiable via on-chain reads — worst case is an undocumented proxy-admin key independent of the 3-of-5 multisig.
...
- **Trust assumption:** the Theo owner multisig — 4-of-6 on Ethereum, truncations of the same owner set elsewhere, and four owners common to every chain satisfy every chain's threshold
...
⚠️ **Restated 2026-08-23 — the thresholds are not the finding, and one of them was wrong in the report's favour.** All four chains were measured with controls passing. Three published figures are **correct**; Ethereum's is not, and it is **stronger** than published:

| chain | published | measured |
|---|---|---|
| **Ethereum** | 3-of-5 | **4-of-6** — higher threshold, and a sixth signer this report omitted (`0x97f60fb7…`) |
| Arbitrum | 3-of-5 | 3-of-5 ✅ |
| Base | 3-of-4 | 3-of-4 ✅ |
| HyperEVM | 3-of-4 | 3-of-4 ✅ |

⚠️ **The differing thresholds are the least interesting property. The owner sets are strictly nested** — measured directly, no chain holds a single owner the others lack:

```
Base / HyperEVM (4)   0x5c1ea222   0x7afb1d33   0x895f7c37   0xb7cc3051
Arbitrum (5)          … the same four …  + 0x12eb20b2
...
**One of those four is `0x7afb1d33…`** — the key that also holds `EMERGENCY_ROLE` on the vault, `CANCELLER_ROLE` on the timelock protecting it, and a seat on the 4-of-5 custody Safe. **Five hats, and the only address in this estate present at every layer measured.** Read that as concentration of *presence*, not of power: it remains one signer among several on each multisig and cannot act alone on any of them.
...
⚠️ **The counterparty is now identified — this open item is closed as to mechanism.** On the same day, thUSD's on-chain reserves fell **$24,987,439** while its supply did not move by a dollar. The thBILL side: **24,131,274 tokens** left supply between 08-20 and 08-22, which at a NAV of 1.035792 is **$24,994,981** — **a match within about $7,500 on $25M**. **thBILL was burned out of the thUSD reserve safe**, which is precisely why no user-facing redemption appears in the record: there was no user. See the [thUSD report](/reports/thusd/), where the same event cut on-chain coverage from 61.22% to 42.86% in one day.

**Two things this does not establish, and the report claims neither.** It does not show **where the $25M of value went**, or what now stands behind that share of thUSD instead. And it does not account for the whole contraction: a **separate earlier tranche of about 1,447,876 thBILL** left supply on 08-20, and thUSD's reserve *rose* $9,093 that day, so that piece did not come out of the same safe and remains unexplained.
...
| layer | what it controls | what governs it | delay |
|---|---|---|---|
| **thBILL's product contracts** | the vault and the OFT adapter | a **48-hour `TimelockController`** (`0x2bb4b7e6…`) as sole `DEFAULT_ADMIN`, with a **4-of-6 Safe** (`0x94877640…01295`) as proposer, executor and canceller | **48 h** |
| thBILL token custody | the 96.3% of supply held intra-protocol | a **4-of-5 Safe** (v1.4.1, Ethereum) | n/a |
| **Custody of the reserve assets** | ULTRA and FILQ themselves | **bare externally-owned keys** — the treasury and the FILQ holder | **none** |
| **The reserve asset's own logic** | the ULTRA token, the largest single leg | **one plain externally-owned key**, sole admin, exercised twice this year | **none** |

⚠️ **The top row is the correction, and it runs the other way from everything else in this section — though not without qualification.** This report described a single "3-of-5 owner multisig" governing contract authority. Measured on Ethereum 2026-08-23, thBILL's **product layer is genuinely well governed**: the vault's sole `DEFAULT_ADMIN` is a **48-hour TimelockController**, self-administered with no external backdoor, and its executor role is **permissioned rather than open to `address(0)`**. The key that administers the ULTRA reserve holds no role on it.

⚠️ **But a complete role census of the vault — every grant from deployment to head, each holder re-confirmed by current-state `hasRole` rather than inferred from logs — found exactly two roles ever granted, and the second one is a plain key.** `DEFAULT_ADMIN_ROLE` is held solely by the timelock. **`EMERGENCY_ROLE` is held solely by `0x7afb1d33…`, which has zero code — an externally-owned account.** It was granted by the 4-of-6 the day after deployment and has never been revoked.

⚠️ **And that key wears three hats at once.** It holds `EMERGENCY_ROLE` on the **$58.5M vault**; it holds `CANCELLER_ROLE` on the **timelock that governs that vault**; and it is an owner on **both** Safes — the 4-of-5 that holds custody and the 4-of-6 that proposes to the timelock. **So the party who can veto a timelocked proposal is the same party holding emergency powers over the thing the timelock protects.** The 48-hour delay is real and well-built, and it has a single-key carve-out standing beside it.

⚠️ **What `EMERGENCY_ROLE` can actually do is not established, and this report does not infer it from the name.** Pause-only would be ordinary and even good design; anything reaching funds would not be. Resolving it requires the implementation's verified source, which this pass could not obtain. **Unexamined, not absent — and "emergency" is a label, not evidence of scope.**

**The honest summary of the product layer, then:** administered by a 48-hour timelock behind a 4-of-6 Safe, **with a single-EOA emergency role of undetermined scope held by a signer who is also the timelock's canceller and sits on both Safes.** Materially better than the ULTRA side, and not unqualified.

**It was also deliberately migrated to, not inherited.** The 4-of-6 Safe granted admin to the timelock on **2026-05-12**, the timelock revoked the Safe's own admin **two days later**, and the deployer was revoked the day after deployment. That is textbook role hygiene, and it is the **same 48-hour timelock that owns thUSD's token** — Theo runs one consistent, well-built product estate.

⚠️ **Which makes the finding worse, not better, and this is the sentence that matters.** The gap is not that Theo lacks the capability to govern this properly — **they built exactly this, in May 2026, and it is still running.** The reserve asset that backs the product is nonetheless administered by a single plain key with no delay and no quorum. **That is a choice about where to apply a practice they already have, not a capability they never acquired.**

And it is not a legacy artifact of one chain: the same lone key is sole admin of `UltraManagerFiat` **on Ethereum**, inside the same estate as the timelocked vault.
...
⚠️ **What this is scored on, and what is genuinely still open — they are different things.**

**Scored now (Project / Counterparty 4.5 → 4.0):** the reserve assets sit at bare externally-owned keys, and this report's admin section accounts for one of three authority layers. **That is settled and depends on nothing outstanding.** Enumerating who holds upgrade authority could make it worse or less bad; it cannot make it untrue. Under the standard applied across this coverage — where the dock is earned by the absence of a reaction window rather than by the number of signatures — a single key that moves $37M with no notice period is squarely on the docked side.

**Also scored now, because the question closed:** the upgrade authority over the reserve asset has been enumerated, and it is a single plain key. **`0x3d5d2bb4…` is an externally-owned account** — zero code, nonce 49 — and it is the **sole `DEFAULT_ADMIN_ROLE` holder on the ULTRA token that backs thBILL, and the sole holder on the vault that holds 97.29% of it.** `getRoleMemberCount` returns **1** on each: not one of several, one in total.

⚠️ **And this is not a dormant capability — it has been exercised four times, directly from that key to the proxy, with no multisig or timelock in between:**

| contract | date | action |
|---|---|---|
| ULTRA token | 2026-03-17 | `upgradeTo` → implementation `0x956a0bda…` |
| ULTRA token | 2026-04-09 | `upgradeTo` → implementation `0x0bd2267b…` (current) |
| Destination vault | 2025-10-27 | `upgradeTo` → implementation `0xd330ba5d…` |
| Destination vault | 2026-03-17 | `upgradeTo` → implementation `0xc2eb2c24…` (current) |

**The token whose logic backs thBILL has been swapped twice this year by one key.** That is the distinction worth holding onto against the rest of this coverage: USDC's undelayed single-key upgrade path, which earned half a notch, is a *capability*. **This is a demonstrated practice.**

**Which sharpens the three-layer point into something worse: the published admin section describes the layer that matters least.** And it raises a question this report cannot answer and will not paper over — **if the 3-of-5 multisig governs neither custody of the reserve nor the upgrade path of the asset backing it, what does it govern?**

Two limits on the above, stated rather than glossed. The role-grant *history* on the ULTRA token is **not** exhaustive — the log scan began at the vault's creation block and the token was deployed earlier, so the original grant falls outside the window. The current-state reads are authoritative and independently controlled, so the conclusion stands, but the history is not a complete record. And a second address holds a named role on the vault from its first block; it appears in none of the addresses this coverage has identified, it is **unexamined**, and this report does not characterise it.

**No restated backing figure appears here either**, but for a different and now-settled reason: the ~$37M understatement is a gap in our own monitoring, and the correction belongs there rather than in this report's basket framing.
...
## IV. Overall Risk Score — 4.4/10

| Category | Score (0-10) | Weight | Key Points |
|---|---|---|---|
| Smart Contract | **5.0** | 40% | **Cut from 5.5 (2026-08-23): a single plain key can replace the logic of the token that backs this product, and has done so twice this year.** See §III. The prior "no timelock" dock priced the 3-of-5 over thBILL's *own* contracts; this is a different and higher layer that was never counted. | Single Zenith audit clean (0C/0H); no bug bounty; OFT expansion likely post-audit; under a year since July 2025 launch. Bridge config verified clean across all four EVM chains — every peered pathway at 3 DVNs OK, zero exposed-AND-peered. Trust holes: undocumented MPC scheme, no pause function on the OFTAdapter |
...
| Project / Counterparty | **4.0** | 30% | **Cut from 4.5 (2026-08-23).** Strong team and regulated institutional partners; fully centralized governance; Panama jurisdiction; no timelock. **The new drag is custody:** the reserve assets sit at **bare externally-owned keys** — about $37M moved from the treasury on a single signature on 2026-08-20, and the $20.1M FILQ leg sits at a second EOA — and the 3-of-5 multisig this report describes governs *contract* authority, not the assets. That is a third authority layer the axis had never counted, and by the standard applied across this coverage it is the absence of a reaction window that earns the dock. |
| **Overall** | **4.4** | | **Cut from 4.8 (2026-08-23): 5.0×0.4 + 4.0×0.3 + 4.0×0.3 = 4.4**, a full notch split across two axes — Smart Contract for the reserve asset's upgrade path, Project / Counterparty for custody of the reserve. Previously cut from 5.1 (2026-08-18) entirely via the Economic axis. Elevated risk — a high-quality asset (US T-bills) wrapped in an early-stage issuer with centralized governance, limited legal remoteness, and a synthetic tULTRA wrapper layer whose 4626 interface is effectively ceremonial. Bridge layer verified clean. What changed is not the asset but the market around it: thBILL now behaves as internal plumbing for thUSD rather than as a standalone product, and its economics depend almost entirely on holding working primary access. BB+ credit equivalent, now at the **lower** end of the band rather than the middle. |
...
thBILL is an elevated-risk tokenized T-bill product with high-quality sovereign underlying assets (delivered via Standard Chartered's Libeara issuer, FundBridge as MAS-regulated fund manager, and Wellington Management as sub-advisor) wrapped in an early-stage issuer with centralized governance and a synthetic tULTRA wrapper layer. Overall score **4.4/10** — cut from 4.8 on 2026-08-23 (a full notch split across Smart Contract and Project / Counterparty, for the reserve asset's single-key upgrade path and for custody of the reserve at bare keys), and before that from 5.1 on 2026-08-18 via the Economic axis alone.
...
- **2026-08-26 — one stale threshold corrected; no score change.** The proxy-admin paragraph still described `0x94877640…01295` as a **3-of-5** — the pre-correction figure that the third and fourth passes below had already replaced, first with 4-of-5 and then with **4-of-6 on Ethereum**. ⚠️ **It survived both corrections because each pass rewrote the admin section and this instance sits about 550 lines above it**, in the upgradeability discussion. The report therefore stated one threshold for this Safe in its admin table and a different one for the same address in its proxy-admin paragraph. **Found by an automated entity-keyed contradiction sweep** (`scripts/check-entity-contradictions.ts`) rather than by reading — it keys on the address, so it compares every claim about one entity regardless of which section makes it. The per-chain figures are unchanged and remain correct: 4-of-6 Ethereum, 3-of-5 Arbitrum, 3-of-4 Base and HyperEVM.
- **2026-08-23 (fourth pass) — per-chain Safe thresholds measured on all four chains; no score change, and Ethereum corrects *upward* again.** Three published thresholds are **correct** — Arbitrum 3-of-5, Base 3-of-4, HyperEVM 3-of-4 — and **Ethereum's is not: it is 4-of-6, not 3-of-5**, with a sixth signer (`0x97f60fb7…`) this report had omitted. ⚠️ **The thresholds are the least interesting part. The owner sets are strictly nested**, measured directly with no chain holding an owner the others lack: four owners on Base and HyperEVM, those four plus one on Arbitrum, those five plus one on Ethereum. So this is **one signer set with three chains holding truncations of it**, not four independently configured Safes — which is what "a small asymmetry worth noting" implied, and that phrasing is retired rather than patched. ⚠️ **The consequence nesting carries and asymmetry would not: the four owners common to every chain satisfy every chain's threshold — Ethereum's 4-of-6 included, because those four sit inside its six. Four keys reach all four chains**, and Ethereum's higher threshold buys nothing against a group that already spans the estate. Same shape as [frxUSD](/reports/frxusd/) but tighter: nested rather than overlapping, so there is not even an extra-signer step to reach the last chain. **One of those four is `0x7afb1d33…`**, which also holds `EMERGENCY_ROLE` on the vault, `CANCELLER_ROLE` on the timelock protecting it, and a seat on the 4-of-5 custody Safe — five hats, and the only address present at every measured layer. **Concentration of presence rather than of power:** it is one signer among several on each multisig and cannot act alone on any. **No score change**, unlike frxUSD: this report already named its cross-chain authority surface and priced it, so this sharpens the description without adding a drag. ⚠️ **A note on method:** the earlier pass measured Ethereum only, and this report declined to propagate that reading to the other three chains. **A bulk replace would have corrupted three accurate rows to fix one.**
