...
*Revision history: 2026-08-29 — **fee switch added; no score change, and deliberately not written as a downgrade.** Ethena tokenholders have approved directing **95% of net revenue** across three business lines to **ENA buybacks** once a USDe supply milestone is reached. ⚠️ **It is dormant: supply is roughly $4.08B against a first milestone reported at $7.5B, so activation needs about +84% growth.** Nothing is diverted today, and treating a dormant commitment as present deterioration would be the harsh-direction error this coverage has hit repeatedly this week. ⚠️ **The finding is what the schedule displaced.** The proposal's own text says the milestones *"would replace any suggested parameters previously suggested by the Risk Committee"* — **and that thread is where the reserve-fund protections lived**: a monthly adequacy assessment gating activation, and an ordering of **Reserve Fund capitalization first**, then sUSDe yields, then sENA. **The replacement text carries no reserve-fund condition.** **Recorded as structural and forward-looking:** the protocol has committed by vote to routing the majority of net revenue outward as it scales, and the reserve-fund-first ordering that governed that no longer applies in the text replacing it. ⚠️ **Two inputs were not retrieved and are not sourced to this report: the milestone table is an IPFS-embedded image, and the Risk Committee's supporting analysis sits in the proposal's replies.** Either could reinstate a reserve-fund gate. **Press-reported tiers are second-hand and used for no judgement here.** **Written to describe the schedule and the supersession rather than an in-flight vote**, so it does not go stale when voting closes. ⚠️ **A What-to-watch line was also replaced:** it tracked the monthly Reserve Fund subcommittee as a forward-positive item, **which is the process now superseded** — so it pointed readers at a venue that no longer governs. 2026-08-25 — **re-verified; nothing moved; scores held at 6.5.** Recorded as a result rather than a silent date move, because a re-read that finds the thesis intact is information and a bumped stamp without one converts an honest staleness signal into a false freshness signal. ⚠️ **Also carried, because the flag names invite the wrong reading:** our monitoring records two Ethena reserve-account gaps — one attested account on **Solana** outside our tracking, and an on-chain wallet read covering **6 of 7** disclosed accounts. **These are record gaps, not exposure gaps.** Neither says anything is missing from Ethena's backing; they say something is missing from *our view* of it, and conflating the two once overstated an apparent shortfall by about seventeen times. `last_verified` is bumped to 2026-08-25 for that material; the cooldown, coverage and secondary-market figures still date from the passes recorded below. 2026-08-23 — corrected the cooldown silo address, no score changes. This report printed the silo as `0x8DE5a6Db…1671` in three places, including step 1 of the redemption instructions and the "what to watch" pointer for reading silo depth. **That address has no code deployed at it.** The live silo is `0x7fc7c91d…3425`, read from `silo()` on the sUSDe vault and holding roughly $25M of USDe; the zero-code result was confirmed on two independent Ethereum RPCs with a positive control in the same batch, so it is a real absence rather than an unresponsive endpoint. **This is a documentation error on our side, not a protocol change** — the cooldown mechanism works as described, `silo()` resolves, and the silo holds real USDe; nothing about Ethena's design or this report's scores is affected. All three references now instruct the reader to resolve the address from `silo()` rather than to trust a written-down constant, which is the durable fix: a hardcoded address cannot survive a redeployment, and a stale one reads as an empty contract rather than as an error. `last_verified` is deliberately **not** bumped — only the silo address was re-verified on 2026-08-23, and the rest of this body still carries its 2026-07-15 reads and the Q1-2026 APY anchor. 2026-07-15 — dynamic cooldown confirmed SHIPPED (Ethena proposal #759): fixed 7-day silo replaced by a coverage-tiered 1/3/5/7-day duration with stress auto-extend, verified on-chain at 1 day. Redemption re-scored 5.5 → 6.5; overall held at 6.5 (USDe-floored). Reconciled underlying USDe float to ~$4.5B (June 2026) and reframed the ~3.72% APY as a Q1-2026 anchor. 2026-07-14 — initial production publish.*
