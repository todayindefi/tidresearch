...
*Revision history: **2026-08-27 — redemption epoch attributed; scores held at 5.0.** The **30-day epoch is the issuer's documented figure, not an on-chain fact** — no `epochDuration`, `currentEpoch`, `epochLength`, `noticePeriod`, `withdrawalDelay` or `cooldownDuration` exists in the deployed bytecode. What is verified is the **ERC-7540 asynchronous structure** (`pendingRedeemRequest`, `claimableRedeemRequest`, `maxRedeem`), so requests queue, settle asynchronously and can roll. **The reconciliation residual is $11.1M, 3.094% of assets**, against $8.40M / 2.35% at the prior read. It stepped on **2026-08-05** from 0.512% to 2.708% and **has not re-converged in twenty-two days** — an earlier and larger step on 2026-06-05 (1.111% → 4.425%) reverted within six. Committed-undrawn escrow is eliminated: it read **$0.00 for the 54 days before the step**. Remaining explanations are unranked. **The undelayed-mint finding on [USDai](/reports/usdai/) does not extend to this vault** — a pair simulation reverts identically (`0xe6c4247b`) from both the operations Safe and a dead address, so it fails a precondition before any access check and nothing about the mint gate is established here. **2026-08-27 — Issuer 6.0 → 5.0**, following USDai, same entity and governance; Overall held at 5.0 on vault-specific drags. **2026-08-18 — chain footprint:** sUSDai is live on Arbitrum, Ethereum, Plasma and Base, not Arbitrum-only. **2026-08-12 — Liquidity 4.5 → 5.0**, Overall held at 5.0. **2026-07-23 — loan book roughly doubled; borrower concentration surfaced**, two borrowers close to the entire pipeline.*