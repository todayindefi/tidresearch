...
**Read the backing claim one hop at a time: a USDat is a claim on $M, and $M is a claim on T-bills.** The first link is the one you can verify yourself in seconds, and it is genuinely excellent. The second is M0's to attest — $M is backed by US Treasury bills held by M0's federation of authorized minters, which is not directly on-chain-verifiable and depends on M0's protocol and its minters posting collateral. Most readers see "T-bill backed" and assume they hold T-bills; here you hold a token that holds them.

M0 is a reputable tokenized-RWA product and there is no evidence of a problem at that layer. But **it is a dependency this coverage has not assessed** — there is no report, score, or admin review of M0 on this site — and it sits under 100% of USDat's reserve. That unexamined layer, not anything visible in the USDat contract, is what keeps the backing axis at 6.5 rather than higher. The collateral-quality read is carried separately on the Underlying axis (5.5), which is where the distance between "verifiably holds $M" and "knows what stands behind $M" is priced.
