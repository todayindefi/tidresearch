...
- **USDat's own contract does not recognise its own collateral.** `isAllowedAsset(PYUSDx)` returns **false**, and `totalAssets()` reports **38.12** — the USDC leg alone, excluding effectively the entire reserve. **This is the sharpest item on the list, and it has nothing to do with PYUSD.** Reporting suggests the migration completes around 2026-08-26, so it may well be transitional; it was the state of the contract on 2026-08-23.
...
- **The rest of the migration landing.** `isAllowedAsset(PYUSDx)` returning false and `totalAssets()` reporting 38.12 are both consistent with a rotation still in progress, and reporting suggests completion around 2026-08-26. Those two values becoming consistent with the actual reserve is the sign it finished. Either of them still wrong well after that date is a different and worse story.
