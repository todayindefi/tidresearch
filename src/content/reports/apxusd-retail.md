...
>
> The figures in the sections below describe a materially worse position than the current one, and the correction matters in both directions. **Current state, re-read 2026-08-23 (feed timestamp 09:45:59Z)** (the figures previously here were as at 2026-08-18): the collateral ratio depends on which basis you use, and there are **three**, not two — each with its own shortfall against a supply of about **$312.1M**:

| basis | ratio | shortfall |
|---|---:|---:|
| Accountable's **attested** headline | **98.02%** | **$6.19M** |
| **our computed backing** | **98.59%** | **$4.40M** |
| our **independent lower bound** | **98.53%** | **$4.59M** |

⚠️ **Pair each ratio with its own deficit.** They are not interchangeable, and quoting one ratio beside another's dollar figure understates or overstates the gap by up to $1.79M — a mistake this report made until 2026-08-23 and has corrected.

**How the three relate, because the ordering is not intuitive.** The lower bound is not a separate measurement: it is **our computed backing with a further haircut applied** — about **$191,000**, stripping the roughly 29 bps wrapper premium at which on-chain STRCx trades over the underlying STRC NAV. So it sits *below* our computed figure and still *above* Accountable's attested one. **The number we can check independently marks better than the attestation even after we haircut it conservatively** — and the reason is that Accountable's headline marks the larger raw-STRC slice at a price it does not disclose. That is a verifiability point, not a solvency one.

⚠️ **The spread between the bases is itself the finding.** The issuer's attestation and our own computation of the same book disagree by **$1.79M**, and the honest presentation is that range rather than a point estimate: **apxUSD is below par on every basis, and the shortfall is somewhere between $4.4M and $6.2M depending on whose mark you trust.**
