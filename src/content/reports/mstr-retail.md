...
MSTR is the common stock of **Strategy Inc.** (formerly MicroStrategy), the publicly listed treasury company holding **~843,775 Bitcoin** (≈$51.7B at ≈$61.3K) on its balance sheet — a stack that *declined* for the first time in the record in the 06-29→07-05 week, when Strategy sold 3,588 BTC to fund preferred dividends, then held flat through the two weeks since (see the update box above). Strategy's legacy enterprise-software business is operationally immaterial; the entire investment thesis and risk profile is the BTC stack + the capital-structure machinery that funds the dividends on a growing preferred-stock obligation.
...
|---|---|---|---|---|
| 843,775 BTC (flat 07-19, 2nd week no sale) | ATM equity ($263.5M in the 07-19 week) + BTC sales authorized | $3.225B (07-19, record) | $15.5B | $6.7B |
...
**For preferred holders (STRC, STRF, etc.):** Strategy now operates a three-leg funding stack — ATM equity issuance, ongoing preferred issuance, and opportunistic BTC sales. Phase 1 is the cash buffer, which has *built* from $871M (5/25) → $1.0B → $1.1B → $1.4B (06-21) → ≈$2.55B (06-28) → $3.0B (07-12) → **a record $3.225B (07-19)**, under the 06-29 framework's **12-month-minimum reserve policy** — common-stock-issuance-funded ($263.5M in the 07-13→07-19 week), with zero forced BTC sales for a second straight week. That eases Phase-1 coverage to roughly **22 months** at current obligation (roughly 30 months on STRC alone) with zero issuance refill, about 10 months of slack above the Board's 12-month floor. The buffer is building even as STRC's price sits below par; that is the key offset to the late-June drawdown, and it is why this stays instrument-price stress rather than a regime change. The same framework adds a **$1.0B STRC-priority buyback** (BTC-funded) as a new tool to defend STRC value directly. Phase 2 is the BTC stack itself — ~31 years of runway at flat BTC and flat preferred outstanding, now with BTC sales formally authorized to fund dividends/reserve/buybacks. The three-leg structure is more robust than a single-leg model would be; residual stress vectors are STRC issuance demand absorption, rate-ceiling headroom, and a future mNAV inversion under sustained BTC weakness.
...
- **mNAV (EV/BTC, currently roughly 0.96 to 0.98 — just below parity, second week).** This is the live signal. After recovering to ~1.01 on the 06-29 8-K, the 8% w/w MSTR selloff into 07-12 pushed it **back just below the 1.0 line** where ATM equity issuance stops being accretive, and it stayed there into 07-19 with MSTR about flat at $94.85. Part of that is mechanical — the growing cash reserve counts against the premium calculation. This is a **watch, not a break**: the cut re-arms only on a **sustained sub-1.0 with the reserve drawing down**, and the reserve is instead building to a record $3.225B — so marks hold. A durable bounce back above ~1.05 reverts the 06-26 parity notch; a sustained sub-1.0 *with reserve drawdown* re-arms the cut; sustained < 0.85 = distress. Use strategy.com's landing-page mNAV as the authoritative reading.
...
- **BTC-sale window date.** Currently ≈2028-05 at zero issuance refill / roughly 22 months (pushed out again by the reserve build to a record $3.225B); drift inward to within ~3 months = acute short-term stress. Extension through successful STRC or ATM cash refill = de-risking.
- **Cash balance.** Currently $3.225B (07-19, a record; built from $871M through $2.55B and $3.0B, now under a 12-month-minimum coverage policy); drops below $500M = stress signal (~3-4 months of cash runway at current obligation).
- **BTC count (currently 843,775, flat for a second week into 07-19).** The BTC Monetization Program is live but has now gone **two consecutive weeks unused** — Strategy funded dividends via common ATM ($263.5M this week) and held BTC flat. Two clean weeks confirm the early-July 3,588-BTC sale was a one-off, not a run-rate. Watch the running pace of any subsequent weekly draws versus the ATM/STRC reserve refills.
- **The August STRC dividend rate, declared in late July.** STRC's mid-$80s trading mechanically points to another increase (to about 12.50%), but the revised late-June policy lets Strategy hold at 12.00% instead and defend STRC via buyback/reserve. A hold would be the first live use of that discretion — mildly negative for anyone relying on the rate ratchet to pull STRC toward par.
