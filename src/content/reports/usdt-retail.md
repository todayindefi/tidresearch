...
**Keep this in proportion, in both directions.** A 3-of-6 of independent keys is a materially harder set to compromise than a single key, and these powers have existed since 2017 and have been used narrowly, overwhelmingly for law-enforcement blacklisting. Nothing here is a bug and nothing suggests misuse. But a reader told that contract risk reduces to "not bugs, and otherwise a transparency question" would not know any of it exists, and on an $88.3B position that is the wrong impression to leave.

⚠️ **Scope this carefully: the above is Ethereum only.** The earlier claim that the contracts are battle-tested "across every chain USDT lives on" was never evidenced, and this pass does not evidence it either. **Tron carries the larger USDT float and runs a different contract that this coverage has not read**, and we have no non-EVM reader. Treat the admin posture on Tron and other non-EVM deployments as unmeasured rather than as either better or worse than Ethereum's.
...
| Issuer | 5.5 | Two resolved regulatory settlements (NYAG + CFTC), BVI domicile with a complex iFinex/Bitfinex structure, no completed full audit, and USDT sits outside the US GENIUS Act framework. Offset by 10+ years operating, improved transparency, and no sustained depeg. ⚠️ **Held at 5.5, but note what this rationale does not include:** every item above is regulatory, structural or disclosure-related, and none of them price the on-chain control surface measured on 2026-08-23 — an undelayed 3-of-6 of plain keys holding `deprecate`, `destroyBlackFunds` and blacklist authority over the Ethereum leg. That is a drag this axis has never counted, and it is deferred to a cohort pass rather than applied here alone; see the note below the table. |
...
> ⚠️ **Why the Issuer axis did not move on the admin finding.** The undelayed 3-of-6 described above is a real drag that this axis has never counted — every item in its rationale is regulatory, structural or disclosure-related. It is nonetheless **deferred rather than applied**, for a reason that is about consistency across this coverage rather than about USDT. We cut [USDC](/reports/usdc/) half a notch for an *undelayed single-key* upgrade path, and USDT's 3-of-6 of independent keys is a materially harder set to compromise than that — so pricing USDT for a better topology than the one that moved USDC would leave the two pages inconsistent with each other. The same measurement has also been published, and deliberately not priced, on [AUSD](/reports/ausd/). **These findings will be scored together in a pass that covers the cohort, so the relative ordering is set once on a common basis rather than one report at a time.** The gap between what is documented here and what is scored is stated openly for that reason; it is not an oversight.
