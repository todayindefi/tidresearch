...
**A fresh read on 2026-08-23 shows the buffer rebuilding rather than draining further.** Six days on from the table above, NAV per share is **0.9936**, total assets are **$80.24M**, and the on-chain buffer is back to about **1.93% of assets** — up from the 1.45% low. That is a real move in the right direction, and it partly answers the question the previous revision left open about whether Saturn would restore the cushion. Keep it in proportion in the other direction too: 1.93% is still inside the same 1.4%-to-2.3% band this vault has oscillated in since midsummer, it is one reading rather than a trend, and a cushion under two percent is a cushion under two percent. The structural point is unchanged — exits are still served from the liquid slice first.
...
- **Anyone who needs a predictable exit date.** The queue settles in a documented 3–7 days *when market conditions permit*, and the buffer that would otherwise absorb a rush is around 1.9% of assets — small enough that the exits ahead of you can consume it, as they did through August.
...
- **The on-chain buffer ratio.** Around 1.9% of assets on 2026-08-23, recovered from a 1.45% low but still inside the band it has held all summer. It was drawn down by redemptions through August and has since been partly rebuilt; whether that continues is the clearest available signal about how seriously Saturn treats stress-period exit.
