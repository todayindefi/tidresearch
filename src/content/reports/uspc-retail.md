...
> **Blue-chip collateral wrapped in a young, ≈$18.5M, near-illiquid token behind an upgradeable single-key (36h-timelocked) proxy.** USPC's underlying funds are about as good as on-chain RWA gets — BlackRock and Fidelity AAA money-market funds plus Apollo's tokenized credit fund, managed by a FINMA-licensed Swiss manager and custodied by an FCA-regulated UK custodian. The risk is everything *around* the assets: a product about five months old whose supply tripled since May while the only DEX pool *shrank* — secondary depth is now just ~2.3% of supply — a single ≈$434K Curve pool with negligible trading as the only retail exit, primary redemption that is institutional/KYC-gated with **US-person eligibility publicly undisclosed**, and both token contracts being upgradeable proxies whose admin keys resolve to one EOA behind a 36-hour timelock. Size any position to your on-chain exit, not to the backing quality.

| Yield (current) | Exit method | Primary redemption | Age | Chain |
|---|---|---|---|---|
| Accrues in the note (NAV-bearing) | Single Curve pool (frxUSD pair) | Institutional/KYC-gated; US-person status undisclosed | ≈5 months | Ethereum |
...
The collateral quality and regulatory wrapper (FINMA manager, FCA custodian) are materially stronger than typical on-chain "synthetic dollars." The risk for an on-chain holder is everything *around* the assets: a ≈$18.5M product about five months old whose supply roughly tripled since late May while secondary depth shrank, a single ≈$434K Curve pool with negligible trading as the only retail exit (now ~2.3% of supply), primary redemption that is institutional/gated and 20%-quarterly-liquid, and both token contracts being upgradeable proxies whose admin keys ultimately resolve to one EOA behind a 36-hour timelock.
...
**2. DEX secondary market (the only path for retail / non-KYC'd holders):** USPC trades in a single Curve `frxUSD/USPC` pool: ≈278K frxUSD + ≈150K USPC, total liquidity ≈**$434K**, imbalanced ~65/35 toward frxUSD, with only a trickle of trading (≈$13K across 3 trades in 24h). There is no CEX listing and no second DEX pool. The thing to sit with: **supply tripled since May (≈$6.8M → ≈$18.5M) but this pool got *smaller* (≈$580K → ≈$434K)** — so on-chain depth fell from ~8.5% to **~2.3% of supply**. Aggregate secondary depth is roughly the size of a small retail position — not the size of a meaningful institutional one, and a bigger holder base now shares it. USPC's secondary print is currently around $0.992, slightly below $1 in the imbalanced pool.

A Spectra fixed-rate market originally cited here matures **02 Jul 2026** (now imminent); fixed-rate wrappers around USPC are a footnote, not a source of exit liquidity, and PT/YT mechanics differ from holding spot.

**Effective exit for size:** gated primary redemption. **Effective exit for retail (especially US-person retail):** the single ≈$434K Curve pool, which means a six-figure exit moves the price hard and a seven-figure exit is not realistically possible without a queue.
...
| Liquidity | 2.5 | Single ≈$434K Curve pool (≈$13K/3-trade 24h volume), imbalanced; no CEX, no second DEX. Depth fell to ~2.3% of a tripled supply, so the case for this score is *stronger* than in May even though the number holds. Effective exit for any meaningful size is gated primary redemption. Worst axis. |
...
| Issuer | 5.0 | Regulated entity stack (Shift Capital BVI / AM Switzerland FINMA / Archax FCA) and a blue-chip underlying — strong frame. Tempered by ~$18.5M scale, <5-month history, undisclosed US-person treatment, KYC gating without published eligibility, no public audit or POR attestation, and no MPC/policy layer disclosed for the controlling EOA. |
...
- **Secondary depth.** A second DEX pool, or the existing Curve pool growing *in step with supply* (rather than shrinking, as it did this quarter), would materially improve the Liquidity axis. Watch the `frxUSD/USPC` pool at `0x96bca2cea58a8e08e7da5c9d68b8acbb28419d1d` — note its depth fell to ~2.3% of supply even as supply tripled.
...
- **Supply growth vs. depth.** This is the one to watch closely: supply has *already* crossed into the low tens of millions (≈$18.5M), but secondary depth did **not** scale with it — the pool shrank. That is the unfavorable half of the scenario flagged last quarter. A reversal (depth scaling up toward double-digit-% of supply) would be a genuinely different conversation; continued divergence is the live risk.
...
*This report is based on Coinshift / Shift Capital public materials and on-chain reads through 2026-06-25. Several material disclosures (US-person redemption eligibility, fees, contract audit, reserve attestation specific to USPC) are not publicly available and would need to be obtained directly from Coinshift. Corrections, attestation links, or additional disclosures welcome at info@tidresearch.com.*
