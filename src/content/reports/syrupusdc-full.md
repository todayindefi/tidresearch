...
**Live dashboard:** [todayindefi.github.io/backing-monitor/?asset=syrupusdc](https://todayindefi.github.io/backing-monitor/?asset=syrupusdc) — refreshed hourly from on-chain reads + Maple GraphQL. Headline metric above the fold is **Pool Collateral Ratio** (init-level weighted average across all active loans, currently ~151.5%) with PCR (`(totalAssets − unrealizedLosses) / totalAssets`) demoted to a small status pill ("PCR 100% · no losses recognized"; turns red on any non-zero `unrealizedLosses`). Treat PCR as a binary loss-recognition alarm, not a leading indicator — by the time it deviates from 100% the loss has already been recognized on-chain. Leading indicators are buffer health (loans below init level) and AUM Coverage 30d (with documented Maple data-quality footnote). The page also surfaces all six structural concerns above as live signals (Set A/B mix, buffer-health, named-issuer roster, deployment ratio, exit slippage, governance topology) plus the DeFi allocation sleeves (delegate-discretionary; no timelock) dormant block, and the Cross-Pool Family panel reconciling concentration with [syrupUSDT](syrupusdt).
