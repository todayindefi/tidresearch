...
mint_paths:
  ...
  - id: "admin_upgrade"
    ...
    trust_size: null
    pausable: true
    timelock_seconds: 259200   # operative delay; MIN_DELAY floor is 86400
...
⚠️ **Base is governed by a different population, not the recurring one.** Base runs its own MCMS instances: all eight MCMS known from Ethereum, Monad and Plasma were `hasRole`-tested against Base's timelock and **none holds any role there.** Base's `RBACTimelock` delay is **3 hours**, execution is **permissionless** through a 621-byte CallProxy, and a **`BYPASSER` role** is held by a single MCMS. **A reader who has verified the Ethereum signer set has verified nothing about Base.**

2. **Role separation among the Safes is real, not nominal — but it does not extend to the timelock's own roles.** The Operational Admin (3-of-5) and Security Admin (3-of-6) are *separate* Safes with *non-overlapping* signer sets, confirmed via on-chain `getOwners()`, so a compromise of one does not automatically compromise the other and the emergency-pause path is structurally separate from the parameter-change path. That much is above-average governance hygiene. **What qualifies it:** the governor timelock's own `ROLE_ADMIN` and `EXECUTOR_ROLE` each include a **bare single key** alongside their Safe, both granted after deployment in one batch (see the role table above). Separation between the Safes is a weaker protection than it looks if a single key can schedule a role grant against the same timelock — subject to the cancellation question this assessment has not resolved.

3. **PoolDelegate is a single-key EOA.** The address Globals registers as the syrupUSDC pool delegate (`0xC1e1...49f`) is an externally-owned account, not a multisig. The delegate role's authority is bounded by the protocol's enforced constraints (overcollateralization rules, asset whitelists set by PoolManager), but within those constraints — funding any loan to any whitelisted borrower at any compliant LTV — the EOA's single key is the only thing standing between operational integrity and an off-script loan. The operational firm behind the EOA almost certainly manages the key with institutional custody (Fireblocks / Copper / similar), but this is not publicly attested. Compromise scenario: an attacker with the EOA key could fund maximum loans to attacker-controlled borrowers (subject to whitelist constraints), drain the pool's free USDC into those loans, then either (a) repay them later for a no-op exfiltration of yield carry, or (b) walk away if the borrower whitelist permits anonymous-to-Maple borrowers. Mitigants: whitelist gating (verify what's enforced on-chain), collateral asset whitelist, the 3-day timelock blocking emergency parameter changes that could expand attack scope, and Security Admin's pause capability.
...
**Contract Risk Score: 7.0/10** — Strong audit profile (Spearbit + Trail of Bits + 6 others), $1M+ Immunefi bounty, ERC-4626 standard with thoughtful Maple v2 modular extension, 3-day timelock on governance (24h floor), non-overlapping multisig signer sets across Operational/Security Admins, clean v2 incident history. Deductions for: PoolDelegate single-key EOA (not a multisig), Strategy contracts may post-date some audits, cross-contract surface is non-trivial, Lindy under v2 is moderate.
...
`liquidityCap` is a `PoolManager` parameter (governance-controlled, 3-day timelock to change) that caps total pool deposits at a dollar maximum. Once TVL hits the cap, new deposits are rejected at the contract layer. Three operational reasons Maple uses one:
...
- **Capture cost estimate:** Compromise of the Timelock proposer set (TBD verify membership), then wait the full 3-day delay for execution, in the open.
...
- **Controls:** **3-day timelock, 24h floor** ✓; on-chain visibility of scheduled proposals; Security Admin pause capability during the window; community ability to exit during the 3-day window if a malicious upgrade is scheduled.
...
**Supply Integrity Score: 6.5/10** — CCIP+CCT bridge architecture is materially stronger than LayerZero OFT (clean track record, RMN backstop, no DVN-collusion attack surface), a 3-day timelock on protocol upgrades, with a 24h floor, is well above peer baseline. Deductions for: per-chain CCT addresses and rate limits not auto-verified pending directory browse, CCIP governance shared-dependency exposure (inherited by every CCIP-bridged asset, not syrupUSDC-specific), Timelock proposer membership not directly readable.
...
**Timelock role holders, enumerated on-chain 2026-08-23** (derived from `RoleUpdated` logs, not from address completion):

| Role | Holders |
|---|---|
| `ROLE_ADMIN` | Safe 4-of-7 `0xd6d4bcde…a196` **+ a bare EOA, `0xa04bddfb5bce1afc0a939749dc721ba762019b2a`** |
| `PROPOSER_ROLE` | Safe 4-of-7 `0xd6d4bcde…a196` |
| `EXECUTOR_ROLE` | Safe 3-of-5 `0xce1ce7c7…34e8` **+ a bare EOA, `0x6d9f3a385796a5407c06a5bf18e903916548993e`** |
| `CANCELLER_ROLE` | Safe 3-of-6 `0x6b1a78c1…0818` |

Both EOAs return empty code, and both return `1` from `hasRole` in Maple's argument order, with a `0xdead` negative control and a live-contract positive control passing in the same batch.

**Two things about that table deserve their own sentences.**

**First, the EOAs were added after deployment, together.** The contract has six `RoleUpdated` events, all grants, none revoked. Four landed at the deployment block (23,418,541). The two bare keys both landed at block 23,428,054 — roughly 9,500 blocks later, in a single batch. This is a deliberate later grant of `ROLE_ADMIN` and `EXECUTOR_ROLE` to two single keys, not a deployment artefact.

**Second, `0xd6d4bcde…a196` is the same 4-of-7 Safe that governs pool permissions across both Syrup pools.** One 4-of-7 therefore covers both who may hold the asset and who may schedule governance.
...
2. **Role separation among the Safes is real, not nominal — and does not extend to the timelock's own roles.** Operational Admin (3-of-5) and Security Admin (3-of-6) are *separate* Safes with *non-overlapping* signer sets; compromise of one does not automatically compromise the other, and the emergency-pause path is structurally separate from the parameter-change path. Above-average hygiene at that layer. But the governor timelock's `ROLE_ADMIN` and `EXECUTOR_ROLE` each also include a bare single key, granted post-deployment — see the role table in §I. Read the Safe separation as real but bounded.
...
*Correction 2026-08-23 — governance timelock restated, no score change. This report described the Maple governor as an "OZ-style Timelock" with a 24-hour delay. Both halves were wrong. It is Maple's own `GovernorTimelock`, not an OpenZeppelin one, and its operative delay is **3 days** (259,200s) with a 2-day execution window; the 86,400s `MIN_DELAY` an earlier pass reported as the delay is in fact the **floor** below which it cannot be set. Re-read on-chain 2026-08-23 with a positive control. **Both corrections favour the protocol**: the visibility window is three times longer than stated, and of five governance timelocks measured across this coverage in August 2026 Maple's is the only one with any floor at all. Also recorded: the role accessor is `hasRole(address, bytes32)` with arguments reversed from OZ, so an OZ-shaped probe reverts and returns nothing — which reads to an independent verifier like an empty role set rather than a wrong signature. And the shared-governance finding is now direct rather than inferred: the two Syrup pools have distinct managers but `governor()` on each returns the same address, and the pool tokens are byte-identical, so holding both diversifies operator risk and not governance risk. **Timelock role holders enumerated for the first time, and the result qualifies a claim this report has carried since publication.** `ROLE_ADMIN` is held by a 4-of-7 Safe plus a bare EOA (`0xa04bddfb…9b2a`); `EXECUTOR_ROLE` by a 3-of-5 Safe plus a second bare EOA (`0x6d9f3a38…993e`); `PROPOSER_ROLE` by the 4-of-7; `CANCELLER_ROLE` by a 3-of-6. Addresses derived from `RoleUpdated` logs rather than completed from truncated strings, with both EOAs confirmed codeless against a negative and a positive control. The two bare keys were granted **after** deployment, together, about 9,500 blocks later — a deliberate act rather than a deployment artefact — and the 4-of-7 is the same Safe that governs pool permissions across both pools. Consequently the report's "role separation is real, not nominal" finding is restated as real **but bounded**: separation between the Operational and Security Safes holds, and does not extend to the timelock's own roles. **Deliberately not asserted:** that role updates are exempt from cancellation, or that no bypass path exists. Neither is verified — the empirical test is closed off because executed proposals return zeroed structs — so the three days is characterised as a **detection** window, with the veto question left explicitly open rather than resolved in either direction. Also recorded: `RoleGranted`, OpenZeppelin's event, returns zero events here while Maple's `RoleUpdated` returns six, so an OZ-shaped enumeration succeeds and reports an empty role set — this governance surface is invisible to conventional tooling, which is why it went unremarked. No axis moved on this pass; the finding sharpens the characterisation of an admin surface already scored as delegate-discretionary, and whether two bare keys on `ROLE_ADMIN` and `EXECUTOR_ROLE` warrant a Contract or Operational move is deferred to a scoring pass that can take the cancellation question with it. `last_verified` is not bumped; the pool figures in this body still date from the 2026-07-02 pass.*
