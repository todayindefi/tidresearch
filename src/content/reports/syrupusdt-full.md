...
3. **Evaluate sizing on credit-quality-vs-benchmark, not historical headline APY.** Plain syrupUSDT pays ~4.5-5% (`coreApy` from `syrupGlobals.apyTimeSeries`), which sits ~50–100 bp above the live 3-month T-bill (~3.7–4.0%, US Treasury fiscal data) and above comparable onchain stablecoin lending (Aave V3 / Morpho USDC + USDT supply ~3.5–4.5%, currently elevated from the mid-April rsETH/Kelp DAO incident). Maple's own GraphQL `usdBenchmarkApy` field reports ~5.2–6.5% but is stale 2023–2024 high-rate-era data and should not be used as a live benchmark — see syrupUSDC institutional report §II yield benchmark table for the corrected reference set. The historical Drips/Seasons incentive layer (active 2024 → Feb 18, 2026) ended for both pools.
