...
- ⚠️ **STILL OPEN, and the directory narrows it rather than closing it — Singapore's stablecoin framework is not established as in force over USDG.** The MAS entry above authorises **Digital Payment Token Service**; **a stablecoin-issuer authorisation is a different thing and was not found listed for this entity.** ⚠️ **Read that as a negative result from a single directory read, not as a positive finding that none exists** — an absence in one lookup is the weakest form of evidence on this page, and it is recorded at that strength deliberately. Secondary sources describe the framework MAS finalised in August 2023 as *expected* to commence in mid-2026; **a dated expectation is not a measurement of today's legal status**, and this report asserts it in neither direction. ✅ **So the precise claim stands as before: "MAS-supervised entity" is a claim about the ENTITY — now verified — and not a claim that the token is regulated under the single-currency-stablecoin regime.** Paxos' own language is that USDG is compliant with the *upcoming* framework, and "upcoming" is doing real work in that sentence.
