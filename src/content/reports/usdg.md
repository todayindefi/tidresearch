...
#     18.1% of supply, which is where the dependency graph actually differs
...
| Liquidity | **4.5** | ⚠️ **The binding constraint is that the chain holding half the supply cannot absorb its own holders.** X Layer carries **47.80% of supply** and its entire local on-chain stable exit is **~$1.79M at −200bps — 0.11% of its own supply**, and it is a wall rather than a worse price. Aggregate sell-side depth is **$17,751,457 = 0.535%**; **OKX is 39–61% of off-chain exit and X Layer is OKX's chain.** Argued under [3 · Liquidity & Exit](#3--liquidity--exit--45) |
| Dependencies | **5.5** | ⚠️ **68.46% of supply on two chains — X Layer 47.80%, Robinhood Chain 20.66% — neither of them Ethereum**, so the operative counterparty for most holders is a chain operator rather than a bank. Level with [syrupUSDC](/reports/syrupusdc/), whose largest single name is 22.32% against USDG's largest leg at more than double that. ⚠️ **Mantle holds $501,103.50 across zero exit venues** — immaterial by size, not by kind. Argued under [4 · Dependencies](#4--dependencies--55) |
...
⚠️ **This composite sits ABOVE the mean of its own axes — 6.5 against 5.83 — and that is deliberate rather than drift.** It is the widest such gap in this corpus, and **24 of 30 production reports sit BELOW their own mean**, so the convention runs the other way. ✅ **The reason it is held: the two axes that carry a fiat stablecoin for a holder — Backing 7.5 and Issuer 7.5 — are intact and measured, and the asset holds attested cash and short-duration government securities at named banks.** ⚠️ **What would change it is a finding on those two**, not further movement on operational control, which is already priced at the floor of this band. **A reader comparing 6.5 to the axis mean is seeing a judgement, not an arithmetic error.**

## 1 · Stability — 8.0

✅ **Measured 2026-09-10: USDG trades at $0.999974 — 0.0026% below par**, and an independent market-cap reading of $3,316,139,084 agrees with the supply measured on-chain to within 0.003%. **USDG is redeemed 1:1 by Paxos against a regulated reserve**, and that direct mint-and-redeem path is what holds the peg rather than any on-chain mechanism — there is no algorithmic stabiliser, no collateral auction and no rebasing. ⚠️ **Primary redemption is institution-gated**, so a retail holder does not use it directly; the peg reaches retail through partner-exchange liquidity and arbitrage against that redeemable floor. ✅ **That is a strong design, and the 8.0 reflects it** — the mechanism's weak point is not the peg but the *access* to it, which is priced on Liquidity & Exit below rather than here.
...
⚠️ **The backing claim rests on a monthly attestation, and it is worth knowing which month.** As at 2026-09-10 the most recent published attestation covers **July 2026** — the transparency page lists January through July for this year. ✅ **That lag is inherent to monthly attestation rather than a lapse**, but it means the reserve composition below is evidenced to July and not to today, and a reader sizing a position on it should know the gap. 
...
**This axis is scored on measured executable depth, and the binding constraint is not the size of the book — it is that the chain holding half the supply cannot absorb its own holders.**

⚠️ **X Layer carries 47.80% of supply — about $1.585B — and its entire local on-chain stable exit is roughly $1.79M at −200bps.** **That is 0.11% of the supply sitting on that chain.** ⚠️ **And the crossing coincides with near-total exhaustion of the counter-asset side, so it is a wall rather than a worse price** — the next dollar does not fill at a discount, it does not fill. **A holder on X Layer must bridge before they can meaningfully sell.**

⚠️ **The route they bridge toward is OKX, and X Layer is OKX's chain.** **Under every denominator examined, OKX is between 39% and 61% of all measured off-chain exit depth.** ✅ **The range is given rather than a single figure because the numerator is agreed and the denominator is not** — two independent aggregations put OKX's own book at **$11,363,257** and **$11,362,715**, agreeing to within 0.005%, while differing on what to divide it by. **The dominant exit venue and the dominant host chain are one counterparty, and that is now a capacity measurement rather than a correlation argument.**

| | measured | share of the $3.32B supply |
|---|---:|---:|
| **Aggregate sell-side depth at 2%** | **$17,751,457** | **0.535%** |
| — centralized venues | $14,020,432 | |
| — on-chain venues | $3,731,025 | |
| X Layer local stable exit at −200bps | ~$1,790,000 | 0.11% *of X Layer's own supply* |

✅ **A construction note, because the direction of a depth figure is easy to get backwards.** Selling USDG is `cost_to_move_down` only where **USDG is the base asset**; where USDG is the **quote** — as in `BTC/USDG` or `ETH/USDG` — the sell-side is `cost_to_move_up`, and the down figure measures someone selling the *other* asset into USDG. ⚠️ **82 of the 99 venues reporting a usable book quote USDG rather than trade against it.** **Summing the down column without that branch measures the mirror image of an exit.** ✅ **The figure above branches on it.** The effect is a 1.1% reduction rather than a large one, because most of those books are near-symmetric — but **the one place it bites is real**: a single ETH/USDG book counted $507,710 of "exit" where the true sell-side is $234,403.

⚠️ **Two venues are excluded, and the exclusions are stated rather than made silently.** One pool reported a two-percent depth larger than USDG's entire supply. Separately, a data vendor reports **$5.74 billion** of USDG reserves on Robinhood Chain — **8.4× the actual local supply** — with several pools returning identical zero-slippage quotes at every trade size from $1,000 to $100M, and others failing outright with "no route found" on a $1,000 trade. ✅ **That defect is venue-specific rather than chain-wide or vendor-wide**: the same vendor's other Robinhood and X Layer pools reconcile against on-chain reserves.

⚠️ **And one deployment has no exit at all.** **Mantle holds $501,103.50 of USDG and has zero venues.** At 0.015% of supply it changes no aggregate — but **a deployment with no exit is a different fact from a small one**, and it is recorded as the former.

✅ **What holds this above the floor:** $17.75M is a real book, 98 venues is genuine breadth, and a holder at retail or ordinary institutional size exits without difficulty today.

⚠️ **One thing to know before comparing this number to others on this site.** **Only five of thirty-three published Liquidity scores rest on a measured depth figure.** **USDT at 9.5, USDC at 9.5, USDS at 8.5 and USDe at 7.0 are size proxies or unstated bases.** ✅ **So the distance between this 4.5 and USDC's 9.5 is not evidence that USDG is five points worse at exit — it is evidence that one of the two was actually measured.** **That is a defect in the ladder rather than in either asset.**

⚠️ **A limit on the agreement above, because it is worth less than it looks.** The two aggregations that agreed to 0.005% were independent *pulls and aggregations* of **the same source**. **A defect in that source would be common to both.** ✅ **The on-chain tick-math ladders — X Layer's $1.79M wall and the reserve reconciliation — are the genuinely independent evidence here**, and they are what the score rests on.
...
**The concentration is the finding, not the bank list.** ⚠️ **68.46% of USDG sits on two chains — X Layer at 47.80% and Robinhood Chain at 20.66% — and neither of them is Ethereum.** For most of the holder base the operative counterparty is a chain operator rather than Paxos' banks. ⚠️ **The comparison against our own anchor for measured, material concentration is unflattering:** [syrupUSDC](/reports/syrupusdc/) sits at this same 5.5 with a largest single name of **22.32%**, and USDG's largest single leg is more than double that. ⚠️ **Mantle's $501,103.50 is immaterial by size and not by kind** — a deployment with zero exit venues is a dependency with no escape, whatever its share. ✅ **Not priced here, and the omission is deliberate:** the Solana key that can mint and seize in one instruction, and the reverting `supplyController()` on every EVM leg, are authority facts that sit at [5 · Contract & Admin](#5--contract--admin--20). **Charging them again would price one thing on two axes.**
...
| chain | supply | share |
|---|---:|---:|
| **X Layer** | **1,585,069,203.94** | **47.80%** |
| Robinhood Chain | 685,184,868.18 | 20.66% |
| Solana | 601,709,953.29 | 18.14% |
| Ethereum | 382,417,182.83 | 11.53% |
| Ink | 60,742,039.18 | 1.83% |
| Arbitrum | 601,242.52 | 0.02% |
| Mantle | 501,103.50 | 0.02% |
| **total** | **3,316,225,593.43** | 100% |
...
- ✅ **RESOLVED 2026-09-10 — Singapore's stablecoin framework is NOT in force, and that is now a positive finding rather than an absence.** **MAS published a Consultation Paper on proposed amendments to the Payment Services Act 2019 on 1 September 2026**, setting out the draft legislation that would implement its single-currency stablecoin regime. **That consultation closes on 16 October 2026, subsidiary legislation is to be consulted on separately afterwards, and no commencement date has been set.** ⚠️ **So the framework is at draft-legislation stage** — not in force, and not close to it. ✅ **This replaces a weaker reading.** The earlier position rested on a stablecoin-issuer authorisation not appearing in MAS' directory, which is an absence in one lookup and the weakest form of evidence; **the consultation paper is direct evidence of the framework's actual stage.** ⚠️ **Read the consequence precisely, because it cuts both ways.** **Paxos' own language — that USDG is compliant with the *upcoming* framework — is accurate, and 'upcoming' is doing real work in it.** USDG is issued under a **Digital Payment Token Service** licence, which is a real licence and a live obligation. **What does not yet exist is the stablecoin-specific regime** — reserve, redemption-timing and no-interest requirements that would bind an issuer as an issuer. **This report's claim has always been that the ENTITY is MAS-supervised, which is verified, rather than that the TOKEN is regulated as a stablecoin, which is not yet possible for anyone.**

---

*Revision history: 2026-09-10 — **whole-body verification; `last_verified` moves for the first time since 2026-07-08.** ⚠️ **Every axis on this report has now been measured rather than carried**, which is what the stamp asserts and why it had been withheld while Liquidity still rested on a size proxy. **Supply re-read across all seven chains on one date: $3,316,225,593.43**, down 0.14% in a day, with the shares moved enough to matter — **X Layer 47.80%** (from 47.98%), **Robinhood 20.66%** (21.07%), **Solana 18.14%** (17.76%), **Ethereum 11.53%** (11.33%). The six EVM legs are **81.86%** of supply, not 82.24%. ✅ **Peg measured at $0.999974, 0.0026% below par**, with an independent market-cap reading agreeing with on-chain supply to 0.003%. ⚠️ **The Backing section now dates its own evidence: the most recent published attestation covers JULY 2026**, so reserve composition is evidenced to July rather than to today — inherent to monthly attestation, but the gap belongs on the page rather than in a reader's assumption. **Peg mechanism, yield structure and the USDC/USDT comparison re-read and unchanged.** No score moved.*
