...
axis_exemptions:
  ...
  - axis: "5 Contract & Admin"
    reason: "⚠️ NOT a coverage gap — all seven chains are read. The authority picture moved three times on 2026-09-09 (timelocked upgrades, then undelayed pause and seizure, then a Solana leg holding more than any EVM leg), and every candidate number so far was derived against a stage that was superseded within the hour. With riskAnalyst for ONE derivation over the whole seven-chain picture. Expect a number, not a permanent blank."
---
...
**No score is attached to Contract & Admin yet, and the reason is no longer a gap in coverage.** ⚠️ **Every chain is now read.** What is outstanding is that the authority picture changed three times in one day — a timelocked upgrade path, then undelayed pause and seizure, then a Solana leg holding more than any EVM leg — and **a number set against any one of those stages would be a number set against a superseded finding.** The axis is with our risk assessment side for a single derivation over the whole seven-chain picture. ⚠️ **A refusal to score is a statement about our confidence and never a reason to withhold a measurement we hold** — so every measurement above is published now, and the number follows once it is derived against all of it.
