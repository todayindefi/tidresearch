...
The second exit is one Curve StableSwap-NG pool, DUSD/frxUSD (A = 1000, 0.01% fee), holding 797,920 DUSD and 2,502,131 frxUSD on 2026-09-21. Measured depth is good: on that date, selling 500,000 DUSD returned 500,465 frxUSD, about +9 bps above par, and on 2026-09-20 even 797,000 DUSD — nearly the whole pool-side balance — quoted at +6.3 bps. Since the entire float outside the pool and protocol contracts is roughly 24,000 DUSD, no plausible holder could move this pool. There is no centralised-exchange listing, and 24-hour volume on 2026-09-20 was $2,116 against a market cap of about $823,000.

**The binding leg of this axis is exit, not depth.** Slippage is not the constraint. What sets the score is that there is no redemption right, there is exactly one venue, and that venue's quote asset is frxUSD rather than a dollar. The realistic path to dollars is DUSD → frxUSD → crvUSD → USDC, three hops: Curve's deepest frxUSD venues pair against crvUSD, sfrxUSD, msUSD and FRAX, and no frxUSD/USDC pool appears among them. The second and third hops have not been measured, so the +9 bps first-hop figure is not the cost of reaching dollars.

**Risks and limitations.** The frxUSD-only exit is a recent condition, not the original design. A direct UniV3 DUSD/USDC pool was created on 2025-12-11, the day after the token, and carried real liquidity for six months; its liquidity reached zero at block 25,329,865 (2026-06-16 11:44 UTC), after which the frxUSD-quoted Curve pool became the only venue. DUSD has therefore traded at par against a real dollar at scale, and no longer can. The depth that remains is substantially the issuer's own: 62% of the Curve pool's LP tokens (2,036,225 of 3,284,183) were minted by the treasury Safe, none has ever been burned, and there is no lockup. The remaining LP sits in Convex and Yearn positions motivated by emissions rather than demand. The pool's current premium to par is a property of its frxUSD-heavy balance and inverts into a discount as soon as the pool turns DUSD-heavy. A treasury withdrawal of LP or a frxUSD stress event would each remove most of the exit at once.
...
- Frax-ecosystem participants who understand DUSD as a frxUSD-denominated instrument, expect to exit through the Curve pool, and check the treasury's LP position and the pool balance before sizing.
- Liquidity providers and borrowers in Alto's markets, for whom DUSD is a protocol unit rather than a savings asset.

## Who should avoid it

- Anyone who needs a redemption right or a dollar-denominated exit. Neither exists for holders.
