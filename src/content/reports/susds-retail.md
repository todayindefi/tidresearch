...
*Revision history: 2026-08-25 — **the 48-hour delay is confirmed and its cancellation leg is qualified; no score change.** Measured on the DSPause at `0xbe286431…38f3`: `delay()` is **172,800 seconds exactly**, `owner()` is the zero address, and `getMinDelay`, `MINIMUM_DELAY` and `MIN_DELAY` all revert. **The delay is real.** ⚠️ **What this coverage had not established, and now has: `plot()` (schedule) and `drop()` (cancel) answer to the same DSAuth authority.** There is no distinct canceller role held by an independent party — so whoever can queue an action can also withdraw one, and **against the scenario a timelock exists to defend against, a compromised or captured proposer, the cancel power belongs to the attacker.** **So the 48 hours buys notice, not interruption.** A holder gets two days of public warning in which to exit, which is real and is why this axis is not docked where undelayed peers are. What a holder does not get is anyone independent who can stop the action inside that window. **Forty-eight hours of notice that nobody independent can act on is notice, not protection**, and this coverage had described the cancellation as though it were the second. **Scores unchanged, and the cohort decision survives the correction:** the discriminating variable across this coverage was always whether a *holder* gets a reaction window, not whether a third party can intervene — and USDS and sUSDS still give one where the docked cases give none. `setDelay` also exists with no floor constant, as previously recorded. `last_verified` is not bumped beyond the governance material, which is what was re-read. 2026-08-23 — no score changes; the inherited governance claim in the Issuer row was independently verified. This report's Issuer rationale cites "48h GSM timelock" as a scoring reason, a fact inherited from [USDS](/reports/usds/) rather than measured here. It was re-read on-chain 2026-08-23 and **holds**: the Sky Governance Security Module delay is exactly 172,800 seconds, the pause contract has no owner backdoor, and no bypass role exists — so the Issuer axis is unchanged, and it stays equal to USDS's, as an entity-scored axis on a wrapper pair must. Recorded because of a freshness trap worth naming: this report carried a 2026-08-18 stamp against USDS's 2026-05-27, so a reader comparing dates would have trusted the fresher page — while the claim on it was an unverified inheritance with less behind it than the older sibling's. **An inherited claim does not inherit the verification date of the report it lands in.** `last_verified` is not bumped; only this one claim was re-checked. 2026-08-18 — **Overall corrected 8.0 → 7.5. The previous 8.0 is superseded, not restated.** sUSDS was publishing above the 7.5 we publish for [USDS](/reports/usds/), the dollar it wraps — a wrapper ranked above its own underlying on the same site. The 8.0 had been argued on a cross-category comparison (strongest of the tokenized-yield products on access and liquidity), which is a fair observation but not a basis for exceeding the underlying. Set equal to USDS rather than notched below it, because the ERC-4626 wrapper adds no material risk beyond USDS. No other axis moved; Liquidity stays 8.5, and USDS was re-reviewed and held at 7.5. Prior: 2026-07-09 — refreshed collateral model (Star-allocator system now ~52%, crypto CDPs ~7%; added credit-sleeve + non-USDC-stable detail); sUSDS issuer axis added (7.0). Overall and other scores unchanged.*
