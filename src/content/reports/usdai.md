...
|---|---|
| from the bridge adapter `0xffA10065Ce1d1C42FABc46e06B84Ed8FfEb4baE5` | **succeeds** |
...
⚠️ **What this does and does not mean, because it is easy to over-read.** It does **not** touch the reserve: an off-Arbitrum mint creates a claim without moving the PYUSD, and coverage remains verifiable by the same two reads. The backing and peg findings above are unaffected. **What changed is the ceiling on how far the operator's discretion can be trusted, not any evidence about the assets.** Against that: the threshold is a genuine 3-of-3 with **zero redundancy**, the protocol has run about ten months with no incidents and no evidence of misuse, and the upgrade timelock remains real and self-administered. **The Issuer axis moves 6.0 → 5.0 and Overall 6.5 → 6.0 (2026-08-26)** — a rescore on a disproved claim, not a re-rating of the reserve. ⚠️ **Overall was cut again the following day, to 5.5, on a separate coverage finding** — see the Overall row for the current figure.

✅ **One adapter address serves every chain.** Permian publishes a single USDai OAdapter — `0xffA10065Ce1d1C42FABc46e06B84Ed8FfEb4baE5` — for Arbitrum, Ethereum, Base and Plasma, and it is confirmed on the first three by reading `token()` back to USDai against a one-character-off control address that holds no code on any of them. ⚠️ **A shared address is not a shared implementation:** the deployed bytecode on Base differs in length from Arbitrum's and Ethereum's, so the mint route is present on each chain rather than identical across them.
