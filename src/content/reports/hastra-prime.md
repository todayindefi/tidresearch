...
There is a second venue of comparable size on Solana — an Orca pool pairing PRIME against PYUSD — where a similar few-million-dollar sale also executes within a few basis points. So a holder willing to split across both chains has meaningfully more room than the Ethereum figures alone suggest, roughly double on the evidence available. The important caveat is that the Solana side has no measured ceiling: the quotes there come from a router aggregating several venues, and its results stop behaving predictably above about $5M, so we can say what executes cheaply but not where it stops. The Ethereum ceiling below is therefore a floor on total exit capacity, not a cap on it.

Pool size is not the same as tradeable size, so the Ethereum exit was measured by quoting actual sale sizes against that pool. Selling PRIME for USDC costs under a basis point for ordinary size and only a few basis points at $5M. Then it stops: once the pool's USDC side is consumed — a little under $6M, and it moves day to day — additional PRIME returns no additional dollars at all. Execution is excellent right up to a cliff, with no gradual worsening to warn a seller they are approaching it. In practical terms a little over 1% of PRIME's market value can leave at a good price on Ethereum and the rest cannot through that pool, so $5M is the sensible ceiling for a single exit on that venue — and that ceiling shrinks whenever the pool rotates toward PRIME. The current figure is on the dashboard, updated hourly.
