...
*Independent backing, reserve segregation, cross-chain supply, redemption-buffer health, warehouse turnover, and liquidity are on the [dashboard](https://todayindefi.github.io/backing-monitor/?asset=hastra-prime), updated daily.*
...
and selling the loans. The Treasury layer pays overnight SOFR minus 0.35% — around
...
unstaking PRIME into wYLDS, then asking Hastra to sell YLDS off-chain for USDC, and
there is no standing buffer to draw on — its balance sits in cents while redemption
requests queue in the hundreds of thousands. Nearly everything in the stack also
...
| | |
|---|---|
| **Backing** | PRIME reconciles to staked wYLDS. At the base layer, FCC reported $601.524M of qualified assets against a $599.706M YLDS reserve at 31 March 2026, while chain reads independently show wrapper coverage above 100%. The measures use different dates and denominators. |
...
The stronger legal description is balanced by limited history and concentration in one origination/securitization channel. The [warehouse monitor](https://todayindefi.github.io/backing-monitor/?asset=hastra-prime), updated daily, tracks inventory and EDGAR ABS-15G securitization cadence. A longer stress record and facility-level delinquency would support a higher mark; stalled turnover or rising losses would move it down.

**Liquidity — 6.0.** Ethereum Uniswap V3 has recently shown under $10M of pool liquidity, with Solana venues alongside it. That gives holders a practical immediate exit for ordinary size and is materially better than wYLDS's standalone market. It also creates a price-discovery path independent of Hastra's primary redemption process.

Pool size is not the same as tradeable size, so the exit was measured by quoting actual sale sizes against the pool. Selling PRIME for USDC costs under a basis point for ordinary size and only a few basis points at $5M. Then it stops: once the pool's USDC side is consumed — a little under $6M, and it moves day to day — additional PRIME returns no additional dollars at all. Execution is excellent right up to a cliff, with no gradual worsening to warn a seller they are approaching it. In practical terms a little over 1% of PRIME's market value can leave at a good price and the rest structurally cannot, so $5M is the sensible ceiling for a single exit — and that ceiling shrinks whenever the pool rotates toward PRIME. The current figure is on the dashboard, updated daily.
...
**Redemption — 5.5.** The primary route is real but layered. A holder requests to unbond PRIME, waits, completes the conversion to wYLDS, then submits a wYLDS-to-USDC request. Pending shares stop earning, and Hastra must fulfill the wrapper request after selling YLDS through Figure Markets. Recent requests cleared 1:1, typically well under an hour. That is useful evidence of normal operation, while the DEX provides a separate immediate route.

Stress capacity is much weaker than the calm-market timing suggests. There is no standing buffer to draw on — the USDC balance sits in cents while redemption requests queue in the hundreds of thousands. The vault also holds about a million YLDS alongside pending-sale amounts. Every material primary exit therefore depends on an administrator, off-chain YLDS liquidity, market-hour processing, and movement of USDC back on-chain. PRIME's extra unbonding step means it does not inherit the base certificate's redemption quality.
...
The 5.5 score reflects that balance: well-documented backing, a thin exit, and no diversification anywhere in the stack. It is also, in practice, mostly collateral: more than half the supply backs leveraged borrowing priced off accrued value rather than market price, which prevents a liquidation spiral but concentrates the consequences of any real decline in that value. PRIME can fit a small private-credit allocation for a holder who understands the HELOC facility, administrator-mediated redemption, and multichain implementation. It is not a cash equivalent, a direct Treasury claim, or an instant trustless redemption product.

**Watch items:** reserve-account segregation; the USDC buffer and pending redemption queue; facility inventory and securitization cadence on the [monitor](https://todayindefi.github.io/backing-monitor/?asset=hastra-prime), updated daily; Figure loan delinquency and any facility-level disclosure; Ethereum pool depth after incentives; the Q2 FCC filing expected around mid-August; and termination or breach of the Figure software licence the protocol runs on.
