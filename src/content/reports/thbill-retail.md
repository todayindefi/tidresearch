...
**Getting in** is easy — buy on a DEX. The deepest venues are typically Project X on HyperEVM (thBILL/USDT0) and Uniswap V3 on Arbitrum (thBILL/USDC); concentration between them shifts over time. The Ethereum Uniswap V3 pool exists but is thin; Base is a deployment chain with no live liquidity. The market price trades **persistently below NAV by tens to low hundreds of bps**, and the discount has been *trending wider* over recent months — from a pre-Apr-2026 baseline near −20 bps to a current regime running in the tens-to-low-hundreds-of-bps range with no mean-reversion mechanism. Live discount + per-chain peg history on [the live dashboard](#live-dashboard).
...
The secondary market prices thBILL at a persistent discount to NAV. This discount is not a bug — it's structural. The only participants who can close it via arbitrage are KYC'd institutions, and they only arbitrage when the spread exceeds their own costs (redemption fees at the underlying fund, gas, 4-day settlement carry). So the discount has a floor, and **that floor is your real exit cost as a retail user.** Empirically, redemptions to date have originated from a single Theo operator address — there is no competitive arbitrage market enforcing the peg, so the discount is not mechanically pulled back.
...
**Stress-event data point (positive — April 27, 2026).** The fund processed a single $65.3M redemption (~33% of supply at the time, the largest on record) cleanly via Theo's primary path — no contract failure, no bridge incident, no backing-ratio break. The structural plumbing held under the largest stress test thBILL has seen. The cost paid was on the secondary peg, not on the redemption rails. So the persistent discount-widening since is the post-stress repricing, not a sign that the underlying mechanics are breaking — but both are real and both matter to your exit math.
