...
**But that is an inference from behaviour, not proof.** The destination contract is built so that whoever can upgrade it isn't readable from the chain, so **this report does not claim Theo controls it, and does not claim anyone else does.** The honest position: the question narrowed from "where did the money go" to "who can upgrade that contract", and the second question is answerable but unanswered. **No restated backing figure is published here until it is** — the plausible answers point in opposite directions, and a number published against evidence that could reverse is worse than no number.
