...
thBILL has been repositioned as a **backing asset for Theo's stablecoin product [thUSD](/reports/thusd)** rather than a consumer-facing T-Bill wrapper. ⚠️ **That repositioning is now close to complete, and it is measurable rather than descriptive.** Read on 2026-08-23: **96.34% of thBILL sits in the thUSD reserve** — against a 55–70% band earlier in the year — leaving an **external float of about 2.14M tokens, roughly $2.2M.** Supply fell 29% in the same window, from 84,059,152 on 08-19 to 58,480,001 on 08-22, and **the reserve's own holding barely changed: what disappeared was almost entirely the float retail could reach.** For a holder that is the single most important number on this page, because everything below about exit, secondary depth and redemption is scoped to that $2.2M — against $621K of pool liquidity and about $4.4K of daily volume. **What caused the move is not established:** no redemption is recorded (the last was 2026-07-09), roughly 25.6M tokens left supply with nothing logged against them, and the mechanism is not visible in on-chain events. This report describes that shape without asserting a cause.
...
⚠️ **But read it as a statement about how little market is left, not about stress.** The float collapse explains what would otherwise look anomalous — a stress-level discount with no stress event: that band was calibrated in April against roughly 28M of external float, and the float is now about 2.14M. A price struck on $621K of pool liquidity and $4.4K of daily volume is not comparable to the same percentage struck on a market ten times deeper. **The honest conclusion is that the old band no longer describes this token**, rather than that thBILL is in distress. That is an interpretation of the timing rather than a proven link. What it means practically is unchanged and simple: **the exit is thin, and getting thinner. Formerly: the market is now more dislocated than it was at the peak of a stress event, with no stress event.** That is not a price signal about the T-Bills; it is what a market looks like when there is no arbitrageur left in it. The mechanism is covered under *Getting out* below — the people who could close this spread have stopped showing up. Live discount + per-chain peg history on [the live dashboard](#live-dashboard).
...
That liquidity does not run "in the low-single-digit-million range" against a fund "on the order of $130M+." Both numbers have fallen, but they have not fallen together: the fund shrank by roughly a third while the tradeable float shrank by considerably more. **The external float has contracted in absolute terms**, which is the expected consequence of thBILL becoming an internal reserve asset — every share that moves into the thUSD reserve is a share that is no longer available to trade against.
