...
#     and only the permissionless-access one is established. The aggregator
#     slippage figure has NO source, date or trade size in our records, and the
#     "queue clears in under five minutes" figure is the ISSUER'S own claim,
#     never independently tested. Free liquidity is 1.08%, below our monitor's
#     2% flag. A depth measurement is commissioned; 7.5 returns only if it
#     lands. Still above syrupUSDT's 6.0 because scale plausibly buys depth.
