...
*Revision history: 2026-08-27 (third pass) — ⚠️ **The second configuration path was checked and is clean; Structural held at 7.0.** A LayerZero *delegate* can alter DVN and verification settings **without `setPeer`**, which is the failure shape that cost Kelp about $292M. **Measured on all five walked legs, delegate equals owner:** Ethereum points at its two-day timelock, and Base, Optimism, BSC and Scroll at their three-day one. **No undelayed configuration path exists on any leg measured.** ⚠️ **Published with the same weight as the correction above, because a negative result here is load-bearing rather than an absence of news.** A reader learning the notice window is two days and not ten must learn in the same breath that **the door beside it is bolted to the same lock** — otherwise the correction reads as a wider hole than it is. **Remaining gap is breadth, not a new path:** ten of fifteen deployments are still unwalked for both `setPeer` owners and delegates, and on every leg checked so far both routes are timelocked. 2026-08-27 (second pass) — ⚠️ **The bridge layer is measured: 4-of-7 into a two-day timelock, not 6-of-10 into ten days. Structural held at 7.0.** The ten-day timelock this report describes governs **upgrades**. The Ethereum OFT adapter — a lockbox holding real weETH — is owned by a **different `TimelockController` with a 172,800-second delay.** Gate-tested: `setPeer` **succeeds** from that two-day timelock and **reverts** from the 6-of-10 upgrade Safe. **The Safe readers were told to trust cannot touch the bridge.** ⚠️ **The bridge proposer shares exactly one owner of seven with that Safe**, so it is effectively a different party and the keys standing behind the bridge number **four, not six**. ⚠️ **This is the route that matters here: the roughly $292M rsETH loss this report narrates was a LayerZero configuration failure, not an upgrade.** The ten days govern the path that did not fail at Kelp; the two days govern the one that did. **Published with the good news rather than after it, because omitting it would misrepresent the finding:** the delay is real, it is **self-administered so it cannot be shortened**, and there is a **genuine independent veto — a 4-of-6 canceller sharing zero owners with the proposer**, which is stronger separation than several assets in this database. **The protection is real, five times shorter than advertised, and held by people this report did not name.** ⚠️ **The axis is held and the cap's basis has changed from *unverified* to *measured*.** Structural 7.0 already discounted "multi-chain bridge exposure untouched and unverified"; that exposure is now quantified and lands roughly where the cap anticipated, so the number stands while the reason no longer rests on an unknown. **Also recorded: the L2 legs run a three-day timelock that also governs upgrades there**, so bridge and upgrade share one authority on Base, Optimism, BSC and Scroll while remaining separate on Ethereum — meaning *"the Ethereum leg is strongest"* is true for upgrades and false for the bridge. **Still unmeasured:** ten of fifteen deployments, and **whether any deployment's LayerZero delegate differs from its owner** — a delegate can alter DVN and verification settings without `setPeer` at all, which is the path closest in shape to the Kelp failure. 2026-08-27 — **a universal claim scoped to what was actually measured; no score change.** ⚠️ **This report contradicted itself on the same page.** The admin section said *"Every privileged action — including contract upgrades — runs through a 6-of-10 Gnosis Safe into a 10-day timelock, re-verified on-chain for this report"*, while the Structural rationale said **the multi-chain bridge exposure is untouched and unverified.** **A universal claim and its own exception, both published, neither referencing the other.** ⚠️ **The sentence that reaches a holder is the one that mattered most:** *"ten days is a long exit window — long enough to unwrap, withdraw and leave."* weETH is a LayerZero OFT across roughly twenty chains, and **this report never measured whether bridge-configuration changes pass through that timelock or are owner-gated with no delay.** **Nothing is asserted about that path here — only that it was not checked**, and that a reader should not read the ten days as covering a surface this report never verified. **The protection a reader is told to rely on must not be broader than the protection that was measured.** ⚠️ **The relevance is not hypothetical, and this report already contained the evidence: the roughly $292M rsETH loss it describes was a LayerZero configuration failure, not an upgrade** — precisely the path the ten-day reassurance does not cover. **The incident, the mechanism and the over-broad sentence were all on this page and none of them referred to the others.** **No axis moves:** Structural is already capped explicitly for unverified bridge exposure, so this is the prose catching up to a drag the score had priced. A measurement of the bridge path is queued separately; **if it comes back clean that is a real result and will be published as one.** `last_verified` unchanged — nothing was re-measured for this. 2026-08-23 — **re-measured; figures updated; thesis unchanged; no score change.** Recording this as a result rather than only moving the date, because on a report whose central argument is that a naive reading invents an event, **a checked-and-held pass is the evidence that the argument survived contact.** At head, with a USDC control passing: `getRate()` **1.101341 → 1.102072**, weETH supply **1,755,436 → 1,823,525**, **ETH-equivalent 1,933,335 → 2,009,656 (+3.95%)** — through 2,000,000 for the first time — and the wrapped share of eETH **95.8% → 96.15%**. **Everything this report argues is more true, not less:** it instructs readers to denominate the position in ETH on the grounds that the USD figure fell while ETH-equivalent grew, and ten further days show ETH-equivalent up again and the wrap share up again. The published 1,933,335 now reads noticeably light, which is why the figures move rather than merely being reaffirmed. ⚠️ **One claim is strengthened rather than restated.** The report derived accrual of about 2.5% annualised from a **single** 79-day observation and hedged it as "over that specific window". An independent, **non-overlapping** 10-day window annualises to **2.45%** — agreement within 5 basis points. The wording moves from one observation to **two**, and the hedge survives the change: two windows is better evidence than one, and it is still two windows on a rate that can move with validator performance or fee policy. ⚠️ **What was NOT re-measured, stated so it is not mistaken for verified-clean:** the **USD figure** (still as at 2026-08-13, and now marked as such inline), the **per-chain rows** and the **Base bridge admin** `0x2f6f…fb68`, and the **weETH/weETHs split dynamics**. `last_verified` is bumped because the rate, supply and wrap share were read at source — the unchecked items above are the boundary of that stamp. *Revision history: 2026-08-13 — initial publication. Reflects the 2026-08-06 removal of restaking from weETH: recategorised from liquid restaking token to liquid staking token, volatility 7.0 to 7.5, structural 6.5 to 7.0, redemption 6.5 to 7.0, overall 6.5 to 7.0, L2 overrides 5.5 to 6.0. Admin chain re-verified on-chain; mainnet implementation noted as changed to `0xA6Ca…ccf3` on an undated earlier upgrade.*
