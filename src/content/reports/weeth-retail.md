...
What that means in plain terms: six of ten named signers must agree to propose any privileged change, and then **everyone gets ten days' notice before it can execute**. Ten days is a long exit window — long enough to unwrap, withdraw and leave if you dislike what has been queued. ⚠️ **That window is established for the mainnet admin path and has not been established for the bridge layer.** weETH is a LayerZero OFT across roughly twenty chains, and **this report has not measured whether bridge-configuration changes — the peer and library settings that determine which messages can credit balances — pass through the same timelock or are owner-gated with no delay.** **We are not asserting that they bypass it. We are saying we did not check, and that a holder should not read the ten days as covering a surface we never verified.** The relevance is not hypothetical: **the roughly $292M rsETH loss this report describes in its bridge section was a LayerZero configuration failure, not an upgrade** — precisely the path this report's strongest reassurance does not cover. For comparison, Lido's equivalent delay is four days, and several LSTs and wrappers in this database have no timelock at all.
