...
The catch: **syrupUSDT is more concentrated than syrupUSDC at the per-pool level, AND it shares borrowers with syrupUSDC.** ⚠️ **Measured 2026-08-23, and the contrast has narrowed:** the largest single loan in syrupUSDT is **38.3% of the pool**, against **23.11%** in syrupUSDC (24.6% on a loans-only basis). This report previously drew that contrast as "~40%+ versus roughly mid-teens", which overstated the gap at both ends. **The conclusion is unchanged — syrupUSDT is the more concentrated pool — but the margin is 38 against 23, not 40-plus against mid-teens.** A single borrower default here still writes down close to two-fifths of the principal in one event.

⚠️ **And there is a sharper way to put it that this report has not been making. syrupUSDT's Herfindahl index is 2,911 — "concentrated" on its own bucket labels — with the top three borrowers at 90.77% across a set of just seven.** That says the pool is **thin all the way down**, not merely top-heavy: removing the largest loan does not leave a diversified remainder. syrupUSDC's equivalent is 1,371 including the Liquidity bucket, or 1,533 loans-only — "unconcentrated" to "moderate" on the same labels, across 16 borrowers.
...
**The clearest way to see the difference is loan count.** At an earlier August 2026 check syrupUSDT's book was **five loans**, against **27** for syrupUSDC — on a pool roughly two-fifths the size. ⚠️ **The book has grown since: the 2026-08-23 read counts seven borrowers in syrupUSDT and sixteen in syrupUSDC**, so treat the five-and-27 figures as the dated snapshot they are and the current loan count as a dashboard number. The *shape* of the point is unaffected and is the durable part. A five-loan book is a categorically different risk shape from a 27-loan one: there is no meaningful diversification within it, one borrower's problem is the pool's problem, and there are fewer repayment events arriving to refill the queue that services redemptions. That single fact is a large part of why this report's **Underlying, Liquidity and Redemption axes all sit below syrupUSDC's** despite identical contracts, identical audits and the same curator. Loan counts turn over; the small-book pattern is the durable feature, and current counts are live on the dashboard.
...
That makes the right sizing unit the **family loan book**, not the per-pool number. Syrup runs a small borrower set (**7 in syrupUSDT, 16 in syrupUSDC** as at 2026-08-23; this report previously said 4 and 13, and the overlap is discussed below), so expect single-counterparty concentration above the ~10%-per-counterparty limit common in institutional credit frameworks. That's the structural product feature, not a temporary state — borrower turnover happens but the small-set + overlap pattern persists.
...
*Revision history: 2026-08-23 — **concentration figures measured and quantified; no score change on either pool.** The sibling contrast has narrowed: largest single loan **syrupUSDT 38.3% vs syrupUSDC 23.11%** (24.6% loans-only), where these reports drew it as "~40%+ versus roughly mid-teens". **The conclusion holds — syrupUSDT is the more concentrated pool — but the margin does not**, and a durable-sounding comparison is a claim to fix rather than a magnitude the "drifts weekly" caveat covers. Borrower counts corrected from 4 and 13 to **7 and 16**. **A stronger statement neither report was making is now on both pages:** syrupUSDT's Herfindahl index is **2,911 — "concentrated" on its own bands — with the top three at 90.77% across seven borrowers**, so the pool is thin all the way down rather than merely top-heavy. syrupUSDC's is **1,371 inclusive / 1,533 loans-only**, unconcentrated-to-moderate, top three 55.5% across sixteen. **Each figure is quoted with its basis**, and the cross-pool comparison is like-for-like. syrupUSDT's Liquidity bucket is about **3.3% of book against syrupUSDC's 6.1%** — the more concentrated pool carrying roughly half the relative cushion — stated as relative size only, since that bucket is pool-owned strategy positions rather than idle cash. ✅ **Credit quality is clean on both: zero impaired, zero called, zero defaulted.** The concentration is a structural exposure, not a problem manifesting. ⚠️ **On syrupUSDC's scores specifically:** the Underlying rationale never counted concentration — it names off-chain custody and Maple's reporting artifact as the two things holding it below 8 — which would normally make this an uncounted drag. **It is not treated as one because the measurement does not support it:** HHI 1,371–1,533 is unconcentrated-to-moderate, credit quality is clean, and the axis is deliberately scoped to collateral quality and verifiability with delegate discretion priced elsewhere. **Cutting for a book that measures unconcentrated would be scoring the prose error rather than the asset.** The hold is made checkable: top-3 rising materially from 55.5%, loans-only HHI crossing 2,500, or any credit event. ⚠️ **Unre-verified rather than refuted:** the named borrower addresses in the cross-pool table are **not** today's top-1 on either pool, and whether they are the same entities has not been checked — **the overlap claim stands unchecked, not disproved.** `last_verified` is not bumped; only the concentration metrics were read.*
