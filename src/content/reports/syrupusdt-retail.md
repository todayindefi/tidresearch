...
The catch: **syrupUSDT is more concentrated than syrupUSDC at the per-pool level, AND it shares borrowers with syrupUSDC.** ⚠️ **Measured 2026-08-23, and the contrast has narrowed:** the largest single loan in syrupUSDT is **38.3% of the pool**, against **23.11%** in syrupUSDC (24.6% on a loans-only basis).h ends. **The conclusion is unchanged — syrupUSDT is the more concentrated pool — but the margin is 38 against 23, not 40-plus against mid-teens.** A single borrower default here still writes down close to two-fifths of the principal in one event.
...
That makes the right sizing unit the **family loan book**, not the per-pool number. Syrup runs a small borrower set (**7 in syrupUSDT, 16 in syrupUSDC** as at 2026-08-23; the overlap is discussed below), so expect single-counterparty concentration above the ~10%-per-counterparty limit common in institutional credit frameworks. That's the structural product feature, not a temporary state — borrower turnover happens but the small-set + overlap pattern persists.
