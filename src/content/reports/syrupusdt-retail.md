...
# shrank by more than a third. Do not diff them. Per-loan collateral visibility
# still dates from 2026-09-06 and predates that contraction. The body states
...
| Liquidity & Exit | 6.0 | **Scored on the worse of the two legs.** The primary leg — permissionless mint and redeem at the vault layer, no KYC — scores 6.5 and is not what sets the axis. Venue depth is the binding one at 6.0. ⚠️ **But that depth has never been measured on either pool.** The comparison this score rested on — higher aggregator slippage than syrupUSDC at institutional sizing — leaned on a syrupUSDC figure since found to carry no source, date or trade size in our records. ⚠️ **And the queue half of that comparison is contradicted by the one leg that is measured:** free liquidity here is **3.39%** at 2026-09-24 against syrupUSDC's **1.08%**, so on cushion this pool queues **less**, not more. **The 6.0 is not moved** — absent a measurement, replacing one unevidenced number with another would invent precision twice. **A depth ladder is commissioned for both pools, and it decides the ordering.** |
...
The loan book is fully enumerated and reconciles to the pool's deployed principal exactly — **17 positions, $244,037,062.37, zero residual**, measured 2026-09-24 — with zero impaired, called or defaulted. **That total is $187,087,047.37 of lending across six borrowers, plus $56,950,015.00 held in nine liquidity positions.** ⚠️ **The cap on this axis is collateral VISIBILITY, not the collateral ratio.** Per-loan collateral comes from Maple's GraphQL API, which returns nothing for a portion of the book, and the loan contracts carry no on-chain collateral field. Existence and principal are verified; collateral is not, and that gap is what holds this axis down rather than anything the collateral ratio says. ⚠️ **The coverage figures themselves date from 2026-09-06 and have NOT been re-measured:** at that read collateral was priced over **63.2%** of the book, and the visible portion was **96.7% crypto** (BTC 74.4%, XRP 22.3%). ⚠️ **The book has contracted 37.9% since, so both figures describe a materially different set of loans and should be read as September measurements rather than current ones.**
...
**Free liquidity is the number that decides how quickly the queue clears, and on this measure syrupUSDT is the better-cushioned of the two pools.** Measured 2026-09-24 as uncommitted cash over total assets — the Liquidity bucket counts as deployed, since those are strategy positions rather than settleable cash — syrupUSDT holds **3.39%** against syrupUSDC's **1.08%**. Both pools are heavily deployed, but syrupUSDT sits in the band where our monitor flags only that *large* exits will queue, while **syrupUSDC is below the 2% level where the monitor flags exits as forced into the queue.** ⚠️ **This is the reverse of what the two Liquidity & Exit scores suggest** — syrupUSDC scores higher on that axis, but on venue depth rather than cushion — and ⚠️ **that depth figure is unmeasured**, so the ordering rests on a claim that has never been tested. **Read the two facts separately rather than inferring one from the other.** Live figures on the dashboard; the durable point is that a small borrower set means few repayment events arriving to refill whichever cushion a pool is running.
