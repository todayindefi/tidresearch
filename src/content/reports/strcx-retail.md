...
**Important context:** Strategy's canonical mNAV (EV/BTC, the metric Strategy publishes) is at **low parity, about 0.98**, so its equity ATM leg is no longer reliably accretive; the compounding engine is roughly neutral. (The "0.86 discount" figure that circulated earlier is the *common-equity coverage* ratio, MktCap/BTC — a leverage lens, not mNAV; with about $22B of debt + preferred sitting ahead of common, that ratio is mechanically below 1 and is not the funding-model signal.) On 2026-05-05 Saylor committed to a **funding mix** — cash on hand first, then opportunistic BTC sales alongside ongoing ATM — for preferred-dividend obligations like STRC; the 06-29 framework formalized that into a standing BTC Monetization Program plus a larger reserve and a $1.0B STRC-priority buyback. Strategy executed the BTC-sale program once (3,588 BTC / about $216M in the 06-29→07-05 week) but has since used common equity for **three straight weeks**, leaving BTC flat at 843,775 while building the cash reserve to a record **$3.75B (07-26)**, roughly 34 months of STRC-dividend coverage. It also used the STRC buyback for the first time, repurchasing 288,930 shares for about $25M at roughly $86.50; about $975M remains. STRC recovered to **about $88.90**, roughly 11% below par, and MSTR rose to about $97.58. The sub-1.0 mNAV is still a watch rather than a break because the reserve is building, and its latest compression partly reflects that cash increase rather than stock weakness. **Critically, the 06-29 framework changed how STRC defends par** — from a reflexive "hike the dividend if STRC trades sub-$95" ratchet to a **discretionary soft floor** (buy STRC back when cheap, or draw the reserve), with no commitment to return STRC to $100. The first buyback turns that soft floor from an authorization into a demonstrated bid. A durable bounce back above about 1.05 mNAV reverts the parity notch; a sustained re-break below 1.0 with the reserve drawing down would re-open the downgrade. See [MSTR report](/reports/mstr/) for the upstream Strategy issuer analysis.
...
- **mNAV (EV/BTC, currently about 0.98 — just below parity).** This is Strategy's published mNAV; ATM equity issuance is accretive while it stays comfortably above 1.0. It remains around the 1.0 line even as MSTR rose to about $97.58, with part of the latest compression caused by the cash reserve increasing to $3.75B because cash is deducted from enterprise value. This is a **watch, not a break** — the downgrade re-opens only on a *sustained* sub-1.0 **with the reserve drawing down**, and the reserve is instead at a record. The binding thresholds: a **durable bounce back above about 1.05** reverts the 06-26 parity notch; a **sustained sub-1.0 with reserve drawdown** re-opens the downgrade; sustained below 0.85 = distress.
- **How STRC's value is defended (soft floor, not par-ratchet).** As of 06-29 a sub-$95 price no longer auto-triggers a dividend hike — Strategy may instead bid for STRC via the **$1.0B STRC-priority buyback** or draw the **$3.75B reserve**. The first buyback is now executed: 288,930 shares for about $25M at roughly $86.50, leaving about $975M. The bid is demonstrated, but still discretionary rather than a contractual par guarantee.
- **The August STRC dividend rate, declared in late July.** STRC's mid-to-high-$80s trading mechanically points to another increase (to about 12.50%), but the revised policy lets Strategy hold at 12.00% and defend via the live buyback and reserve instead. A hold remains a real possibility.
...
- **BTC-sale window (currently roughly 25 to 26 months at zero issuance refill).** Drift toward about 3 months = acute short-term stress. Extension through successful STRC or ATM cash refill (the reserve build to a record $3.75B pushed it out further) = de-risking. The BTC Monetization Program is live but has now gone **three consecutive weeks unused** — Strategy funded dividends via common ATM (about $544M in the 07-20→07-26 week) with BTC flat at 843,775. Management said it will not buy more Bitcoin until STRC returns to $100, so the current pause is deliberate. Watch the *pace* of any subsequent weekly draws versus reserve refills.
