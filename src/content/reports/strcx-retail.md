...
Strategy held a **record $3.225B cash** as of the 07-20 8-K — a buffer that has *built* from $871M (5/25) → $1.0B → $1.1B → $1.4B → $2.55B → $3.0B → $3.225B (07-19), under the 06-29 framework's **12-month-minimum reserve policy**, funded by common-stock issuance ($263.5M this week) with zero forced BTC sales for a second straight week. Against the current ~$130-140M/month obligation, that is roughly **22 months until BTC sales operationally begin** (roughly 30 months on STRC-dividend service alone) if Strategy does not refill cash through STRC or ATM issuance. The buffer is building even as STRC's price sits below par — that is the key offset to the late-June drawdown, and it is why this stays instrument-price stress rather than a credit event. The 06-29 framework also formalized **BTC sales** (a standing Monetization Program — executed once in the 06-29→07-05 week, when Strategy sold 3,588 BTC / ~$216M into dividend service dropping the stack 847,363 → 843,775, then **unused in the two weeks since** as the funding mix flipped back to equity) and a **$1.0B STRC-priority buyback** as additional defenses. The zero-refill assumption makes the cash-exhaustion date a worst-case watermark, and once the buffer runs out, the BTC stack picks up.
...
- **Anyone sizing without a runway view.** Strategy's BTC-stack runway is currently ~31 years (aggregate preferred service), comfortable at first glance — but the near-term cash-exhaustion date is roughly 22 months out at zero issuance refill (eased by the reserve build to a record $3.225B), and the long-term anchor compresses fast under BTC drawdowns or continued preferred issuance growth. Size as if your STRC exposure can recur indefinitely, but also as if the BTC stack is the binding constraint that could halve in BTC bear markets.
...
- **mNAV (EV/BTC, currently roughly 0.96 to 0.98 — just below parity, second week).** This is Strategy's published mNAV; ATM equity issuance is accretive while it stays comfortably above 1.0. It recovered to ~1.01 on the 06-29 8-K, then the 8% w/w MSTR selloff into 07-12 pushed it **back just below 1.0**, where it stayed into 07-19 with MSTR about flat at $94.85 — partly a mechanical effect of the growing cash reserve, which counts against the premium calculation. This is a **watch, not a break** — the downgrade re-opens only on a *sustained* sub-1.0 **with the reserve drawing down**, and the reserve is instead at a record $3.225B. The binding thresholds: a **durable bounce back above ~1.05** reverts the 06-26 parity notch; a **sustained sub-1.0 with reserve drawdown** re-opens the downgrade; sustained < 0.85 = distress. (Note: the MktCap/BTC "coverage" ratio of ~0.86 that circulated earlier is a leverage lens, not mNAV — don't read it as discount.)
- **How STRC's value is defended (soft floor, not par-ratchet).** As of 06-29 a sub-$95 price no longer auto-triggers a dividend hike — Strategy may instead bid for STRC via the **$1.0B STRC-priority buyback** or draw the **$3.225B reserve**. Watch buyback execution + reserve coverage as the live soft-floor signals, not the old "$95 → +50bp" reflex. A dividend *cut* (rate down while STRC is well below par) would signal Strategy leaning fully on buyback/reserve instead of the coupon.
- **The August STRC dividend rate, declared in late July.** STRC's mid-$80s trading all month mechanically points to another increase (to about 12.50%), but the revised policy lets Strategy hold at 12.00% and defend via buyback/reserve instead. A hold would be the **first live use of that discretion** — mildly negative if you are relying on the rate ratchet to pull STRC back toward par.
...
- **BTC-sale window (currently roughly 22 months / ≈2028-05 at zero issuance refill).** Drift toward ~3 months = acute short-term stress. Extension through successful STRC or ATM cash refill (the reserve build to a record $3.225B pushed it out further) = de-risking. The BTC Monetization Program is live but has now gone **two consecutive weeks unused** — Strategy funded dividends via common ATM ($263.5M in the 07-13→07-19 week) with BTC flat at 843,775; the early-July 3,588-BTC sale was a one-off. Watch the *pace* of any subsequent weekly draws versus reserve refills.
