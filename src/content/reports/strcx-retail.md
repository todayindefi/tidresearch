...
Strategy held a **record $3.0B cash** as of the 07-13 8-K — a buffer that has *built* from $871M (5/25) → $1.0B → $1.1B → $1.4B → $2.55B → $3.0B (07-12), under the 06-29 framework's **12-month-minimum reserve policy**, funded by common-stock issuance (ATM resumed at $466.7M this week) with zero forced BTC sales. Against the current ~$130-140M/month obligation, that is roughly **20 months until BTC sales operationally begin** (~28 months on STRC-dividend service alone) if Strategy does not refill cash through STRC or ATM issuance. The buffer is building even as STRC's price sits below par — that is the key offset to the late-June drawdown, and it is why this stays instrument-price stress rather than a credit event. The 06-29 framework also formalized **BTC sales** (a standing Monetization Program — executed once in the 06-29→07-05 week, when Strategy sold 3,588 BTC / ~$216M into dividend service dropping the stack 847,363 → 843,775, then **not used in the 07-06→07-12 week** as the funding mix flipped back to equity) and a **$1.0B STRC-priority buyback** as additional defenses. The zero-refill assumption makes the cash-exhaustion date a worst-case watermark, and once the buffer runs out, the BTC stack picks up.
...
- **Anyone sizing without a runway view.** Strategy's BTC-stack runway is currently ~31 years (aggregate preferred service), comfortable at first glance — but the near-term cash-exhaustion date is ≈20 months out at zero issuance refill (eased by the reserve build to a record $3.0B), and the long-term anchor compresses fast under BTC drawdowns or continued preferred issuance growth. Size as if your STRC exposure can recur indefinitely, but also as if the BTC stack is the binding constraint that could halve in BTC bear markets.
...
- **mNAV (EV/BTC, currently ~0.97–0.98 — just below parity).** This is Strategy's published mNAV; ATM equity issuance is accretive while it stays comfortably above 1.0. It recovered to ~1.01 on the 06-29 8-K, then the ~8% w/w MSTR selloff into 07-12 pushed it **back just below 1.0**. This is a **watch, not a break** — the downgrade re-opens only on a *sustained* sub-1.0 **with the reserve drawing down**, and the reserve is instead at a record $3.0B. The binding thresholds: a **durable bounce back above ~1.05** reverts the 06-26 parity notch; a **sustained sub-1.0 with reserve drawdown** re-opens the downgrade; sustained < 0.85 = distress. (Note: the MktCap/BTC "coverage" ratio of ~0.86 that circulated earlier is a leverage lens, not mNAV — don't read it as discount.)
...
- **BTC-sale window (currently ≈20 months / ≈2028-03 at zero issuance refill).** Drift toward ~3 months = acute short-term stress. Extension through successful STRC or ATM cash refill (the reserve build to a record $3.0B pushed it out further) = de-risking. The BTC Monetization Program is live but was **not used in the 07-06→07-12 week** — Strategy funded dividends via resumed common ATM ($466.7M) with BTC flat at 843,775; the prior week's 3,588-BTC sale was a one-week event. Watch the *pace* of any subsequent weekly draws versus reserve refills.
