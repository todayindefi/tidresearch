...
*Revision history: 2026-08-24 — initial publication at Overall 4.0 (Volatility 5.0 / Liquidity 7.0 / Counterparty 4.5 / Redemption 5.0). Written because STRC was the only asset indexed on the dashboards page without a published report, while sitting underneath three covered assets — and because its only prior coverage was inside the report on its *wrapper*, which accounts for about a quarter of the exposure.

**On the exposure figures, which are the most quotable lines here and the most easily overstated.** Of roughly $264.4M attributed across the three assets, **$66.63M is measured** — an on-chain STRCx balance times a market price — and **about $197.8M is inferred by subtraction**: Apyx's raw STRC is its disclosed STRC bucket less the on-chain wrapper, custodied off-chain at a brokerage, and Saturn's reproduces `total assets − on-chain buffer` exactly, in a field our monitoring names *implied* and flags `oracle_unverified`. **The total is therefore two issuer attestations plus one on-chain balance, and is attributed to the issuers rather than presented as an observation.** An earlier draft of this report quoted it as a measured figure; that was corrected before publication.
