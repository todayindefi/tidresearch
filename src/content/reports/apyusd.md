...
⚠️ **The size of that custody door is stated in tokens rather than dollars, deliberately.** The Safe holds **705,956 STRCx — 42.0% of STRCx supply** — read on-chain 2026-09-09. **A dollar mark on it depends on which STRC price you use and on the date you use it**, and this coverage has carried a figure that appears to have picked up a separate off-chain raw-STRC balance instead. **The token count is the durable number; a USD figure for this door should be quoted with its price basis and its date or not at all.**
