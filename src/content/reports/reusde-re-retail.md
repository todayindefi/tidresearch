...
|---|---|---|---|---|
| 12.27% APY (Re's own simple annualisation, 7d to 2026-08-27) | Curve REUSDE/sUSDe pool | Quarterly windows (72h), 40-day min hold, pro-rata | ~11 months | Ethereum (measured: 12 chains probed, code on Ethereum only) |
...
Re-measured on-chain **2026-08-27**: reUSDe has **$19.85M TVL** across **14,094,070 tokens** at a NAV of **1.408519**, on Ethereum only (no multi-chain deployment, no CEX listing) — contract [`0xddc0f880…cc5a`](https://etherscan.io/address/0xddc0f880ff6e4e22e4b74632fbb43ce4df6ccc5a). **The NAV is not issuer-reported:** Re's own `/tvl` implies 1.408519 against measured supply and **CoinGecko independently prints $1.41**, 0.03% apart. NAV has grown from $1.00 at inception — about **41% lifetime appreciation**, reflecting the higher spread plus sUSDe yield on idle capital.
...
⚠️ **And the NAV has stepped down once, which this report previously had no evidence of either way.** Re's own `/price` series shows reUSDe falling **$1.203409 to $1.184704, −1.55%, across a single quarter boundary (2025-06-30 to 2025-07-01)** — **the only down-day in 485 observations**, against zero in 430 for the senior tranche. **Publish it as what it is: an unexplained down-step, not a confirmed loss.** Re states no underwriting year has reached the mezzanine, and the shape — a single step at quarter close — is more consistent with an actuarial restatement of tNAV than a claim pass-through. ⚠️ **But it sits awkwardly beside the never-impaired framing, and it is direct evidence that this NAV does step down at quarter close.** A holder should expect that mechanism to exist, whatever caused this instance.
...
- **Holder count: not measured** (was 327 in May). No free indexer reaches it; `eth_getLogs` is range-capped.
